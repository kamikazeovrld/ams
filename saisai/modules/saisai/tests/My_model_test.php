...
class My_model_test extends Tester_base {
	...
	{
		$this->load_sql('test_saisai_schema.sql', 'saisai');
		$this->load_sql('test_generic_schema.sql', 'saisai');
	...
	public function test_foreign_key_relationship()
	{
		...
		// set the foreign key
		$test_custom_records_model->foreign_keys = array('role_id' => array(SAISAI_FOLDER => 'saisai_categories_model'));
		
		// grab a user
		$user = $test_custom_records_model->find_by_key(1);

		// setup category data
		$categories_model = $this->saisai->categories->model();
	...
	public function test_has_many_relationship()
	{
		...
		$test_custom_records_model->has_many = array('tags' => array(SAISAI_FOLDER => 'saisai_tags'));
		...
		// setup tags
		$tags_model = $this->saisai->tags->model();
		...
		$test_custom_records_model->has_many = array('tags' => array(
																	'model' => array(SAISAI_FOLDER => 'saisai_tags'),
		...
		$test = is_a($user_model, 'Saisai_tags_model');
		$expected = TRUE;
		$this->run($test, $expected, 'has_many model object returned test');

		$users = $user_model->db()->where(array('saisai_tags.name' => 'Yoda'));
		$test = count($users);
		$expected = 1;
	...
	{
		$test_custom_records_model = new Test_users_model();
		$tables = $this->CI->saisai->config('tables');
