...
class Saisai_cache_test extends Saisai_test_base {
	...
	public function test_saving_cache()
	{
		...
		$this->saisai->cache->save($cache_id, $to_cache);
	...
	public function test_is_cached()
	{
		// cache a file to test against
		$cache_id = $this->saisai->cache->create_id();
		$test = $this->saisai->cache->is_cached($cache_id);
		$expected = TRUE;
	...
	public function  test_page_cache()
	{
		...
		$location = 'test-cache';
		$page = $this->saisai->pages->create($location);
		$page->layout = 'none';
		...
		$cache_id = $this->saisai->cache->create_id($location);
		$test = $this->saisai->cache->is_cached($cache_id, $this->saisai->config('page_cache_group'));
		$expected = TRUE;
		...
		$this->saisai->cache->clear_compiled();
