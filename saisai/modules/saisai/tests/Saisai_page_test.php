...
class Saisai_page_test extends Saisai_test_base {
	...
	public function test_save_new_page()
	{
		...
		$page = $this->load_page('saisai/pages/create', $post);
	...
	public function test_edit_page()
	{
		...
		$page = $this->load_page('saisai/pages/edit/1', $post);
		$test = $this->_is_success();
		$expected = TRUE;
		$this->run($test, $expected, 'Test for successful edit of page');

		// check if page contains archived values
		$test = $this->page_contains('#saisai_restore_version option[value="1"]', TRUE);
		$expected = TRUE;
