 ...
 * SAISAI Helper
 * 
 * Contains SAISAI specific functions. This helper is automatically loaded with the autoload config.
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/helpers/saisai_helper
 */

// --------------------------------------------------------------------

/**
 * Returns the instance of the SAISAI object
 *
 * @access	public
 * @param	mixed
 * @return	string
 */
function &saisai_instance()
{
	return Saisai::get_instance();
}

/**
 * Returns the instance of the SAISAI object just like saisai_instance but with a similar syntax to the CI() function which returns the CI object.
 * 
 * @access	public
 * @param	mixed
 * @return	string
...
<ul>
	...
	<li><strong>language</strong>: the language version to use for the block. Must be a value specified found in the 'languages' options in the SAISAI configuration</li>
 ...
 * @return	string
 */
function saisai_block($params, $vars = array(), $check_db = TRUE, $scope = NULL)
{
	$CI =& get_instance();
	return $CI->saisai->blocks->render($params, $vars, $check_db, $scope);
}
 ...
 * If SAISAI's configuration mode is set to either <dfn>auto</dfn> or <dfn>cms</dfn>, then it will first look for data from the SAISAI navigation module.
 ...
 * @return	string
 */
function saisai_nav($params = array())
{
	$CI =& get_instance();
	return $CI->saisai->navigation->render($params);
}

// --------------------------------------------------------------------

/**
 * Generates a page using the Saisai_page class
 *
 * @access	public
 * @param	array
 * @param	array
 * @return	string
 */
function saisai_page($location, $vars = array(), $params = array())
{
	$CI =& get_instance();
	return $CI->saisai->pages->render($location, $vars, $params, TRUE);
}
 ...
 * @return	string
 */
function saisai_form($fields, $values = array(), $params = array())
{
	$CI =& get_instance();
 ...
 * @return	string
 */
function saisai_model($module, $params = array(), $where = array())
{
	$CI =& get_instance();
	$module = $CI->saisai->modules->get($module, FALSE);
	if ($module)
	{
 ...
 * 
 * Using saisai_set_var in a layout field in the admin, will have no affect (e.g. {saisai_set_var('layout', 'my_layout')}).
 *
 * @access	public
 * @param	string
 * @param	array
 * @return	string
 */
function saisai_set_var($key, $val = NULL)
{
	$CI =& get_instance();
	if (strtoupper($CI->saisai->config('double_parse')) == 'AUTO')
	{
		$CI->saisai->set_config('double_parse', TRUE);
	}
...
<code>
	...
	...
	&lt;php
	saisai_var_append('css', 'my_css_file.css');
	saisai_var_append('js', 'my_js_file.js');
	?&gt;
 ...
 * @return	void
 */
function saisai_var_append($key, $value)
{
	$CI =& get_instance();
...
because it returns inline editing markers that later get parsed out by SAISAI. For example:</p>

<code>
// NO
&lt;a href="&lt;?=site_url(saisai_var('my_url'))?&gt;"&gt;my link&lt;/a&gt;

// YES
&lt;?=saisai_edit('my_url', 'Edit Link')?&gt; &lt;a href="&lt;?=site_url($my_url)?&gt;"&gt;my link&lt;/a&gt;
</code>
 ...
 * @return	string
 */
function saisai_var($key, $default = '', $edit_module = 'pagevariables', $evaluate = FALSE)
{
	$CI =& get_instance();
	...
	if (!empty($edit_module) AND $CI->saisai->pages->mode() != 'views' AND !defined('SAISAIIFY') OR (defined('SAISAIIFY') AND SAISAIIFY))
	{
		$marker = saisai_edit($key, humanize($key), $edit_module);
	}
	else
	{
		$marker = '';
 ...
 * @return	string
 */
function saisai_edit($id, $label = NULL, $module = 'pagevariables', $is_published = TRUE, $xoffset = NULL, $yoffset = NULL)
{
	$CI =& get_instance();
	$page = $CI->saisai->pages->active();
	$permission = $module;
	if (empty($page))
	{
		$page = $CI->saisai->pages->create();
	}
	if (!empty($id) AND (!defined('SAISAIIFY') OR defined('SAISAIIFY') AND SAISAIIFY !== FALSE))
	{
		if (is_object($id) AND is_a($id, 'Data_record') AND isset($id->id))
		{
			...
			if (empty($module) OR $module == 'pagevariables')
			{
				...
				unset($tables);
				$mod = $CI->saisai->modules->get($module, FALSE);
				if (!empty($mod))
				{
...
/**
 * 	Creates the cache ID for the saisai page based on the URI.
 * 
 ...
 * @return	string
 */
function saisai_cache_id($location = NULL)
{
	$CI =& get_instance();
	return $CI->saisai->cache->create_id($location);
}

// --------------------------------------------------------------------

/**
 * Creates the admin URL for SAISAI (e.g. http://localhost/MY_PROJECT/saisai/admin)
 *
 * @access	public
 * @param	string
 * @param	boolean
 * @return	string
 */
function saisai_url($uri = '', $query_string = FALSE)
{
	$CI =& get_instance();
	$uri = saisai_uri($uri, $query_string);
	return site_url($uri);
}
...
/**
 * Returns the SAISAI admin URI path
 *
 * @access	public
 * @param	string
 * @return	string
 */
function saisai_uri($uri = '', $query_string = FALSE)
{
	$CI =& get_instance();
	...
	return $CI->saisai->config('saisai_path').$uri.$q;
}

// --------------------------------------------------------------------

/**
 * Returns the uri segment based on the SAISAI admin path
 *
 * @access	public
 * @param	int
 * @param	boolean
 * @return	string
 */
function saisai_uri_segment($seg_index = 0, $rerouted = FALSE)
{
	$CI =& get_instance();
	if ($rerouted)
	{
		return $CI->uri->rsegment(saisai_uri_index($seg_index));
	}
	else
	{
		return $CI->uri->segment(saisai_uri_index($seg_index));
	}
}
...
/**
 * Returns the uri index number based on the SAISAI admin path
 *
 * @access	public
 * @param	int
 * @return	int
 */
function saisai_uri_index($seg_index = 0)
{
	$CI =& get_instance();
	$saisai_path = $CI->saisai->config('saisai_path');
	$start_index = count(explode('/', $saisai_path)) - 1;
...
/**
 * Returns the uri string based on the SAISAI admin path
 *
 * @access	public
 * @param	int
 * @param	int
 ...
 * @return	string
 */
function saisai_uri_string($from = 0, $to = NULL, $rerouted = FALSE)
{
	$CI =& get_instance();
	$saisai_index = saisai_uri_index($from);
	
	if ($rerouted)
	{
		$segs = $CI->uri->rsegment_array($saisai_index);
	}
	else
	{
		$segs = $CI->uri->segment_array($saisai_index);
	}
	$from = saisai_uri_index($from);
	if (isset($to))
	{
		$to = saisai_uri_index($to);
		if ($from < $to)
		{
			$to = $from;
		}
 ...
 * @return	boolean
 */
function is_saisaiified()
{
	$CI =& get_instance();
	return $CI->saisai->auth->is_saisaiified();
}

// --------------------------------------------------------------------

/**
 * Check to see if you are in the SAISAI admin
 *
 * @access	public
 * @return	boolean
 */
function in_saisai_admin()
{
	return (defined('SAISAI_ADMIN') AND SAISAI_ADMIN === TRUE);
}
 ...
 * @return	boolean
 */
function saisai_user_lang()
{
	$CI =& get_instance();
	return $CI->saisai->auth->user_lang();
}
 ...
 * @return	mixed
 */
function saisai_settings($module, $key = '')
{
	$CI =& get_instance();
	return $CI->saisai->settings->get($module, $key);
}

/* End of file saisai_helper.php */
/* Location: ./modules/saisai/helpers/saisai_helper.php */
