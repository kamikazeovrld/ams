 ...
 * @category	Helpers
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/helpers/my_url_helper
 */
 ...
 * @param	boolean	sets the language parameter on the URL based on the "language_mode" setting in the SAISAI configuration
 * @return	string
 */
function site_url($uri = '', $https = NULL, $language = NULL)
{
	...
	else
	{
		...
		if (isset($CI->saisai))
		{
			$uri  = $CI->saisai->language->uri($uri, $language);
		}
...
function uri_path($rerouted = TRUE, $start_index = 0, $strip_lang = TRUE)
{
	...
	if ($strip_lang AND isset($CI->saisai) AND $CI->saisai->language->has_multiple())
	{
		$segments = $CI->saisai->language->cleaned_uri_segments(NULL, $rerouted);
	}
	else
	{
...
{
	$CI =& get_instance();
	$CI->saisai->redirects->execute($redirect);
}
...
/* Location: ./modules/saisai/helpers/MY_url_helper.php */
