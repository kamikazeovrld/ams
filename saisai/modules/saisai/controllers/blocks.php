...
class Blocks extends Module {
	...
	public function import_view()
	{
		...
		if (!empty($_POST['id']))
		{
			$out = $this->saisai->blocks->import($this->input->post('id'), $this->sanitize_input);
		}
		...
		if (!empty($_POST) AND !empty($_FILES))
		{
			...
			if ($this->upload->do_upload('file'))
			{
				...
				else
				{
					...
					$this->saisai->admin->set_notification(lang('blocks_success_upload'), Saisai_admin::NOTIFICATION_SUCCESS);
					
					redirect(saisai_url('blocks/edit/'.$id));
				}
	...
	public function layout_fields($layout, $id = NULL, $lang = NULL, $_context = NULL, $_name = NULL)
	{
		...
		$layout = $this->saisai->layouts->get($layout, 'block');
		if (!$layout)
		{
			return;
		}
		...
		if (!empty($id))
		{
			...
			if (empty($model))
			{
				...
				$page_vars = $this->saisai_pagevariables_model->find_all_by_page_id($id, $lang);
