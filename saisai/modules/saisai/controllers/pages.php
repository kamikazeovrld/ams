...
class Pages extends Module {
	...
	{
		parent::__construct();
		$this->load->module_model(SAISAI_FOLDER, 'saisai_pagevariables_model');
	}
	...
	public function create($field = NULL, $redirect = TRUE)
	{
		...
		if (isset($_POST['id'])) // check for dupes
		{
			...
			else
			{
				...
				// grab the layout
				$layout = $this->saisai->layouts->get($this->input->post('layout', TRUE));
		...
		$this->saisai->admin->render('pages/page_create_edit', $vars);
	}
	...
	public function _form($id = NULL)
	{
		...
		$this->saisai->load_model('saisai_navigation');
		...
		{
			$layout = $this->saisai->layouts->default_layout;
		}
		...
		******************************************************************************/
		...
		if (!empty($id))
		{
			...
			if (!$lang) $lang = $this->saisai->language->default_option();
			$page_vars = array_merge($this->saisai_pagevariables_model->find_all_by_page_id($id, $lang), $page_vars);

			// the following will pre-populate fields of a different language to the default values
			if (empty($page_vars) AND $this->saisai->language->has_multiple() AND $lang != $this->saisai->language->default_option())
			{
				$page_vars = $this->saisai_pagevariables_model->find_all_by_page_id($id, $this->saisai->language->default_option());
			}
		...
		$vars['form_action'] = ($this->saisai->admin->is_inline()) ? $this->module_uri.'/inline_'.$action_uri : $this->module_uri.'/'.$action_uri;
		$vars['versions'] = $this->saisai_archives_model->options_list($id, $this->model->table_name());
		...
		{
			$this->saisai->admin->add_recent_page($this->uri->uri_string(), $vars['data'][$this->display_field], $this->module);
		}
		return $vars;
	}
	...
	public function _page_var_fields($layout, $posted)
	{
		...
		foreach($fields as $key => $val)
		{
			...
			if (isset($val['type']) AND $val['type'] == 'block' AND isset($posted[$key]['block_name']))
			{
				...
				$block_layout = $this->saisai->layouts->get($posted[$key]['block_name'], 'block');
				if ($block_layout)
				{
							...
							foreach($posted[$key] as $a => $b)
							{
								if (is_array($b))
								{
									foreach($b as $c => $d)
									{
										...
										{
											$block_layout = $this->saisai->layouts->get($d['block_name'], 'block');
											if ($block_layout)
											{
												$block_fields = $block_layout->fields();
	...
	public function _save_page_vars($id, $vars, $fields)
	{
		...
		$this->saisai_pagevariables_model->delete($delete);
		$pagevariable_table = $this->db->table_info($this->saisai_pagevariables_model->table_name());
		...
		foreach($fields as $key => $val)
		{
			...
			if (!in_array($val['type'], $non_recordable_fields))
			{
				...
				if (!$this->saisai_pagevariables_model->save($save, $where))
				{
		...
		$page_variables_archive = $this->saisai_pagevariables_model->find_all_array(array('page_id' => $id));
		...
		if ($this->input->post('navigation_label')) {
			...
			$save['group_id'] = $this->saisai->config('auto_page_navigation_group_id');
			$save['parent_id'] = 0;
			...
			$does_it_exist_already = $this->saisai_navigation_model->record_exists($where);
			if (!$does_it_exist_already)
			{
				...
				if (!empty($parent_location)) $parent = $this->saisai_navigation_model->find_by_location($parent_location);
				if (!empty($parent)) {
					$save['parent_id'] = $parent['id'];
				}
				$this->saisai_navigation_model->add_validation('parent_id', array(&$this->saisai_navigation_model, 'no_location_and_parent_match'), lang('error_location_parents_match'), '{location}');
				$this->saisai_navigation_model->save($save, array('location' => $this->input->post('location', TRUE), 'group_id' => $save['group_id']));
			}
		}
		...
		{
			$this->saisai->cache->clear_page($this->input->post('location', TRUE));
		}
		return TRUE;
	}
	...
	public function layout_fields($layout_name, $id = NULL, $lang = NULL, $vars = array())
	{
		...
		$layout = $this->saisai->layouts->get($layout_name);
		if (!$layout)
		{
			return;
		}
		...
		if (!empty($id))
		{
			$pagevars = $this->saisai_pagevariables_model->find_all_by_page_id($id, $lang);

			// the following will pre-populate fields of a different language to the default values
			if (empty($pagevars) AND $this->saisai->language->has_multiple() AND $lang != $this->saisai->language->default_option())
			{
				$pagevars = $this->saisai_pagevariables_model->find_all_by_page_id($id, $this->saisai->language->default_option());
			}
	...
	public function import_view()
	{
		if (!empty($_POST['id']))
		{
			...
			$pagevars = $this->saisai->pages->import($this->input->post('id', TRUE), $this->sanitize_input);
	...
	public function select()
	{
		...
		$this->load->library('form_builder');
		$pages = $this->saisai->pages->options_list();
		$pdfs = $this->saisai->assets->dir_files('pdf', TRUE);
		...
		$this->saisai->admin->set_panel_display('notification', FALSE);
		$this->saisai->admin->set_titlebar($crumbs);
		$this->saisai->admin->render('modal_select', $vars);
	}
	...
	public function upload()
	{
		...
		if (!empty($_POST) AND !empty($_FILES))
		{
			...
			if ($this->upload->do_upload('file'))
			{
				...
				$id = $this->input->post('id', TRUE);
				$pagevars = $this->saisai->pages->import($id);
				...
				if (!empty($pagevars))
				{
					...
					foreach($pagevars as $key => $val)
					{
						$where['page_id'] = $id;
						$where['name'] = $key;
						$page_var = $this->saisai_pagevariables_model->find_one_array($where);
						$save['id'] = (empty($page_var['id'])) ? NULL : $page_var['id'];
						$save['name'] = $key;
						$save['page_id'] = $id;
						$save['value'] = $val;
						if (!$this->saisai_pagevariables_model->save($save))
						{
					...
					$page = $this->saisai_pages_model->find_by_key($id, 'array');
					...
					if (!has_errors())
					{
						...
						$this->saisai->admin->set_notification(lang('pages_success_upload'), Saisai_admin::NOTIFICATION_SUCCESS);
						
						redirect(saisai_url('pages/edit/'.$id));
					}
		...
		$vars['form'] = $this->form_builder->render();
		$vars['back_action'] = ($this->saisai->admin->last_page() AND $this->saisai->admin->is_inline()) ? $this->saisai->admin->last_page() : saisai_uri($this->module_uri);
		//$vars['back_action'] = saisai_uri($this->module_uri);
		
		$crumbs = array($this->module_uri => $this->module_name, '' => lang('action_upload'));
		$this->saisai->admin->set_titlebar($crumbs);
		
		$this->saisai->admin->render('upload', $vars);
	}
	...
	public function refresh_field()
	{
		...
		if (is_ajax() AND (!empty($_POST) OR !empty($_GET)))
		{
			...
			$layout_obj = $this->saisai->layouts->get($layout);
	...
	protected function _process_upload_data($field_name, $uploaded_data, $posted)
	{
		...
		foreach($uploaded_data as $key => $val)
		{
			...
			if (isset($field_name) AND !is_array($this->model->key_field()) AND isset($posted['page_id']))
			{
				...
				$id = $posted['page_id'];
				$where = array($this->saisai_pagevariables_model->table_name().'.page_id'=> $id, 'name' => $field_name);
				$data = $this->saisai_pagevariables_model->find_one_array($where);
				...
				if ($save)
				{
					...
					$this->saisai_pagevariables_model->save($data);
				}
			}
		}
	}
