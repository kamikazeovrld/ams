...
class Generate extends Saisai_base_controller {
	...
	public function advanced($name = NULL)
	{
		...
		$saisai_config = $this->saisai->config('generate');
		...
		$this->saisai->permissions->create($name);
	...
	protected function _advanced($name = NULL)
	{
		...
		if (!isset($saisai_config['advanced']))
		{
			show_error(lang('error_missing_generation_files', 'advanced'));
		}
		
		$config = (array)$saisai_config['advanced'];
		...
		if (!in_array($name, $this->saisai->config('modules_allowed')))
		{
			...
			{
				$this->errors[] = lang('error_could_not_create_file', $my_saisai_path)."\n";
			}
			$this->modified[] = $my_saisai_path;
			
			// save to database if the settings is there
			
			$module_obj = $this->saisai->modules->get($name);
			if (!empty($module_obj) AND $this->saisai->modules->is_advanced($module_obj))
			{
				...
				{
					$this->saisai->settings->save(SAISAI_FOLDER, 'modules_allowed', $modules_allowed);
				}
			}
		}
	...
	/**
	 * Generates the table, model, permissions and adds to MY_saisai_modules
	 *
	 * @access	public
	...
	/**
	 * Protected helper method that generates the table, model, permissions and adds to MY_saisai_modules
	 *
	 * @access	public
	 * @param	string	Model name
	 * @param	string	Module name (optional)
	...
	protected function _simple($name = NULL, $module = '')
	{
		...
		// check that the configuration to map to files to generate exists
		if (!isset($saisai_config['simple']))
		{
			show_error(lang('error_missing_generation_files', 'simple'));
		}
		
		$config = (array)$saisai_config['simple'];

		$basepath = (!empty($module)) ? MODULES_PATH.$module.'/' : APPPATH;
		
		
		// add to MY_saisai_modules if it doesn't exist'
		$module_path = (!empty($module)) ? $basepath.'config/'.$module.'_saisai_modules.php' : $basepath.'config/MY_saisai_modules.php';
		...
		$this->saisai->permissions->create_simple_module_permissions($name);
	}
	...
	public function _model($model, $module = '')
	{
		...
		$config = (array)$saisai_config['model'];
	...
	protected function _load_results($vars)
	{
		...
		{
			$this->load->module_view(SAISAI_FOLDER, '_generate/results_cli', $vars);
		}
		else
		{
			$crumbs = array(lang('module_generate'));
			$this->saisai->admin->set_titlebar($crumbs);
			$this->saisai->admin->render('_generate/results', $vFuears, Saisai_admin::DISPLAY_NO_ACTION);
		}
	}
	...
	protected function _find_template($file, $vars, $type = 'advanced')
	{
		...
		$search = (array)$saisai_config['search'];
