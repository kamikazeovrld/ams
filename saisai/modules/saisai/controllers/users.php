...
class Users extends Module {
	...
	public function edit($id = NULL, $field = NULL, $redirect = TRUE)
	{
		...
		if (!empty($user))
		{
			...
			if (!$this->saisai->auth->is_super_admin() AND is_true_val($user['super_admin']))
			{
				show_404();
			}
		}
		...
		if (!empty($_POST['permissions']) AND !$this->saisai->auth->has_permission('permissions'))
		{
	...
	public function login_as($id, $original_user_hash = '')
	{
		...
		$this->load->module_model('saisai', 'saisai_users_model');
		$change_logged_in_user = $this->saisai->auth->is_super_admin();
		...
		{
			$curr_user = $this->saisai->auth->user_data();
			$valid_user = $this->saisai_users_model->find_one_array(array('id' => $id));
			$this->saisai->auth->set_valid_user($valid_user);
	...
	{
		$valid_user = $this->saisai->auth->valid_user();
		$can_publish = ($heading == 'active' AND $this->saisai->auth->has_permission($this->permission) AND $cols['id'] != $valid_user['id']);
