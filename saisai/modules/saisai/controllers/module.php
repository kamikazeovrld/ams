...
class Module extends Saisai_base_controller {
	...
	function __construct($validate = TRUE)
	{
		...
		$this->load->module_model(SAISAI_FOLDER, 'saisai_archives_model');
		if (empty($this->module))
		{
			$this->module = saisai_uri_segment(1);
		}
		...
		$params = array();
		if ($this->saisai->modules->exists($this->module, FALSE))
		{
			$this->module_obj = $this->saisai->modules->get($this->module, FALSE);
			$params = $this->module_obj->info();
		}
		else if ($this->saisai->modules->exists($this->module.'_'.saisai_uri_segment(2), FALSE))
		{
			// if it is a module with multiple controllers, then we'll check first and second SAISAI segment with an underscore'
			$this->module = $this->module.'_'.saisai_uri_segment(2);
			if ($this->saisai->modules->exists($this->module, FALSE))
			{
				$this->module_obj = $this->saisai->modules->get($this->module, FALSE);
				$params = $this->module_obj->info();
		...
		else if ($this->saisai->modules->exists(saisai_uri_segment(2), FALSE))
		{
			$this->module = saisai_uri_segment(2);
			$this->module_obj = $this->saisai->modules->get($this->module, FALSE);
			...
			if (empty($mod_name))
			{
				show_error(lang('error_missing_module', saisai_uri_segment(1)));
			}
			unset($mod_name);
			$params = $this->module_obj->info();
		}
		...
		if (!empty($this->language)) 
		{
			if (is_array($this->language))
			{
				...
				// we load the main language file automatically with the Saisai_base_controller.php
				$this->load->module_language($lang_module, $lang_file, $this->saisai->auth->user_lang());
			}
			else
			{
		...
		$this->saisai->admin->load_js_localized($params['js_localized']);
	...
	function items()
	{
		...
		// added per http://www.saisai.co/forums/discussion/760/filter-dropdown/#Item_2
		...
		if ($this->saisai->language->has_multiple() AND !empty($this->language_col) AND method_exists($this->model, 'get_languages'))
		{
		...
		// save page state
		$this->saisai->admin->save_page_state($params);
		...
		if (!is_ajax())
		{
			...
			$config['base_url'] = saisai_url($this->module_uri).'/items/?'.$query_str;
			$uri_segment = 4 + (count(explode('/', $this->module_uri)) - 1);
			$config['total_rows'] = $this->model->list_items_total();
			$config['uri_segment'] = saisai_uri_index($uri_segment);
		...
		if (is_ajax())
		{
			...
			$has_edit_permission = $this->saisai->auth->has_permission($this->permission, "edit") ? '1' : '0';
			$has_delete_permission = $this->saisai->auth->has_permission($this->permission, "delete") ? '1' : '0';
			
			// set data table actions... look first for item_actions set in the saisai_modules
			$edit_func = '
			$CI =& get_instance();
			$link = "";';
			if ($has_edit_permission)
			{
				...
				{
					$url = saisai_url("'.$this->module_uri.'/edit/".$cols[$CI->model->key_field()]);
			...
			if ($has_delete_permission)
			{
				...
				{
					$url = saisai_url("'.$this->module_uri.'/delete/".$cols[$CI->model->key_field()]);
					$link = "<a href=\"".$url."\" class=\"action_delete\">".lang("table_action_delete")."</a>";
					$link .= " <input type=\"checkbox\" name=\"delete[".$cols[$CI->model->key_field()]."]\" value=\"1\" id=\"delete_".$cols[$CI->model->key_field()]."\" class=\"multi_delete\"/>";
				}';				
			}
			...
			foreach($this->table_actions as $key => $val)
			{
				...
				{
					if ($this->saisai->auth->has_permission($this->permission, "edit"))
					{
						$action_url = saisai_url($this->module_uri.'/edit/{'.$this->model->key_field().'}');
				...
				else
				{
					if (strtoupper($val) != 'VIEW' OR (!empty($this->preview_path) AND strtoupper($val) == 'VIEW'))
					{
						...
						$action_url = saisai_url($this->module_uri.'/'.strtolower($val).'/{'.$this->model->key_field().'}');
...
//			$this->data_table->only_data_fields = array_merge(array($this->model->key_field()), $this->model->boolean_fields);
			...
			$has_publish_permission = ($this->saisai->auth->has_permission($this->permission, 'publish')) ? '1' : '0';
			$has_edit_permission = $this->saisai->auth->has_permission($this->permission, 'edit') ? '1' : '0';
			...
			$this->data_table->auto_sort = TRUE;
			$heading_sort_func = (isset($this->disable_heading_sort) AND $this->disable_heading_sort) ? '' : 'saisai.sortList';
			...
			$this->load->module_view(SAISAI_FOLDER, '_blocks/module_list_table', $vars);
			return;
		}
		...
		else
		{
			...
			$vars['table'] = $this->load->module_view(SAISAI_FOLDER, '_blocks/module_list_table', $vars, TRUE);
			...
			$vars['actions'] = $this->load->module_view(SAISAI_FOLDER, '_blocks/module_list_actions', $vars, TRUE);
			...
			$this->saisai->admin->set_inline($inline);
			
			if ($inline === TRUE)
			{
				$this->saisai->admin->set_display_mode(Saisai_admin::DISPLAY_COMPACT_TITLEBAR);
			}
			$this->saisai->admin->render($this->views['list'], $vars);
		}
	}
	...
	protected function _list_process()
	{
		...
		$page_state = $this->saisai->admin->get_page_state($this->module_uri);
	...
	function create($field = NULL, $redirect = TRUE)
	{
		...
		if (!$this->saisai->auth->module_has_action('save'))
		{
			show_404();
		}
		
		// check permissions
		if (!$this->saisai->auth->has_permission($this->module_obj->permission, 'create'))
		{
			show_error(lang('error_no_permissions'));
		}
		
		$inline = $this->saisai->admin->is_inline();
		...
		if (isset($_POST[$this->model->key_field()])) // check for dupes
		{
			if ($id = $this->_process_create() AND !has_errors())
			{
				if ($inline === TRUE)
				{
					$url = saisai_uri($this->module_uri.'/inline_edit/'.$id);
				}
				else
				{
					$url = saisai_uri($this->module_uri.'/edit/'.$id);
				}
				...
				if ($redirect)
				{
					if (!$this->saisai->admin->has_notification(Saisai_admin::NOTIFICATION_SUCCESS))
					{
		...
		if ($inline === TRUE)
		{
			$this->saisai->admin->set_display_mode(Saisai_admin::DISPLAY_COMPACT);
		}

		$vars['actions'] = $this->load->module_view(SAISAI_FOLDER, '_blocks/module_inline_actions', $vars, TRUE);
		$this->saisai->admin->render($this->views['create_edit'], $vars);
		return $id;
	}
	...
	{
		$this->saisai->admin->set_inline(TRUE);
		$this->create($field);
	}
	...
	protected function _process_create()
	{
		...
		else
		{
			...
			if (!$this->saisai->auth->has_permission($this->permission, 'publish'))
			{
				$posted['published'] = 'no';
				$posted['active'] = 'no';
			}
	...
	{
		// check that the action even exists and if not, show a 404
		if (!$this->saisai->auth->module_has_action('save') AND  $this->displayonly === FALSE)
		{
			show_404();
		}

		// check permissions
		if (!$this->saisai->auth->has_permission($this->module_obj->permission, 'edit') AND !$this->saisai->auth->has_permission($this->module_obj->permission, 'create'))
		{
			show_error(lang('error_no_permissions'));
		}
		...
		if ($this->input->post($this->model->key_field()))
		{
			if ($this->_process_edit($id) AND !has_errors())
			{
				if ($inline === TRUE)
				{
					$url = saisai_uri($this->module_uri.'/inline_edit/'.$id.'/'.$field);
				}
				else
				{
					$url = saisai_uri($this->module_uri.'/edit/'.$id.'/'.$field);
				}
		...
		if (!empty($field))
		{
			$this->saisai->admin->set_display_mode(Saisai_admin::DISPLAY_COMPACT_NO_ACTION);
		}
		else if ($inline === TRUE)
		{
			$this->saisai->admin->set_display_mode(Saisai_admin::DISPLAY_COMPACT);
		...
		$vars['actions'] = $this->load->module_view(SAISAI_FOLDER, '_blocks/module_create_edit_actions', $vars, TRUE);
		$this->saisai->admin->render($this->views['create_edit'], $vars);

		// do this after rendering so it doesn't render current page'
		if (!empty($data[$this->display_field]) AND $inline !== TRUE)
		{
			$this->saisai->admin->add_recent_page($this->uri->uri_string(), $this->module_name.': '.$data[$this->display_field], $this->module);
		}
	...
	protected function _sanitize($data)
	{
		...
		if (!empty($this->sanitize_input))
		{
			...
			$valid_funcs = $this->saisai->config('module_sanitize_funcs');
	...
	protected function _shell_vars($id = NULL, $action = 'create')
	{
		...
		// other variables
		$vars['id'] = $id;
		$vars['versions'] = ($this->displayonly === FALSE) ? $this->saisai_archives_model->options_list($id, $this->model->table_name()) : array();
		$vars['others'] = $this->model->get_others($this->display_field, $id);
		$vars['action'] = $action;
		
		$vars['module'] = $this->module;
		$vars['notifications'] = $this->load->module_view(SAISAI_FOLDER, '_blocks/notifications', $vars, TRUE);
	...
	protected function _form_vars($id = NULL, $values = array(), $field = NULL, $inline = FALSE)
	{
		...
		if (!empty($field))
		{
			...
			// added per pierlo in Forum (http://www.saisai.co/forums/discussion/673/saisai_helper-saisai_edit-markers)
		...
		if (is_array($fields))
		{
			...
			{
				if (!$this->saisai->auth->has_permission($this->permission, $k))
				{
					unset($fields[$v]);
				}
			...
			$fields['__saisai_inline_action__'] = array('type' => 'hidden');
			$fields['__saisai_inline_action__']['class'] = '__saisai_inline_action__';
			$fields['__saisai_inline_action__']['value'] = (empty($id)) ? 'create' : 'edit';
			
			$fields['__saisai_inline__'] = array('type' => 'hidden');
			$fields['__saisai_inline__']['value'] = ($inline) ? 1 : 0;
			...
			// we will set this in the BaseSaisaiController.js file so that the jqx page variable is available upon execution of any form field js
	...
	function form($id = NULL, $field = NULL)
	{
		...
		$this->load->module_view(SAISAI_FOLDER, '_layouts/module_form', $vars);
	}

	function delete($id = NULL)
	{
		// check that the action even exists and if not, show a 404
		if (!$this->saisai->auth->module_has_action('delete'))
		{
			show_404();
		}
		
		if (!$this->saisai->auth->has_permission($this->permission, 'delete'))
		{
			show_error(lang('error_no_permissions'));
		}
		...
		if (!empty($_POST['id']))
		{
			...
			if (count($posted) > 1)
			{
				$this->saisai->logs->write(lang('module_multiple_deleted', $this->module));
			}
			else
			{
				$this->saisai->logs->write(lang('module_deleted', count($posted), $this->module));
			}
			...
			if ($inline)
			{
				...
				$vars['id'] = '';
				$vars['back_action'] = '';
				$this->saisai->admin->render('modules/module_close_modal', $vars);
				$this->saisai->admin->set_display_mode(Saisai_admin::DISPLAY_COMPACT_TITLEBAR);
				$this->saisai->admin->render($this->views['delete'], $vars);
			}
			else
			{
				...
				if ($any_success)
				{
					...
					{
						$this->saisai->admin->set_notification(lang('data_deleted'), Saisai_admin::NOTIFICATION_SUCCESS);
					}
				}
				...
				if ($any_failure)
				{
					...
					$this->saisai->admin->set_notification($msg, Saisai_admin::NOTIFICATION_ERROR);
				}
				
				$url = saisai_uri($this->module_uri);
				redirect($url);
			}
		}
		else
		{
			...
			{
				$this->saisai->admin->set_display_mode(Saisai_admin::DISPLAY_COMPACT_NO_ACTION);
				$vars['back_action'] = saisai_url($this->module_uri.'/inline_edit/'.$id);
			}
			else
			{
				$this->saisai->admin->set_display_mode(Saisai_admin::DISPLAY_NO_ACTION);
				$vars['back_action'] = saisai_url($this->module_uri.'/');
			}
	...
	function restore()
	{
		...
		if (!empty($_POST['saisai_restore_version']) AND !empty($_POST['saisai_restore_ref_id']))
		{
			if (!$this->model->restore($this->input->post('saisai_restore_ref_id'), $this->input->post('saisai_restore_version')))
			{
				$msg = lang('module_restored', $this->module_name);
				$this->saisai->logs->write($msg);
				
				$this->saisai->admin->set_notification($this->model->get_validation()->get_last_error(), Saisai_admin::NOTIFICATION_ERROR);
			...
			else
			{
				if (!$this->session->flashdata('success'))
				{
					$this->saisai->admin->set_notification(lang('module_restored_success'), Saisai_admin::NOTIFICATION_SUCCESS);
				}
				$this->_clear_cache();
			}
			redirect(saisai_uri($this->module_uri.'/edit/'.$this->input->post('saisai_restore_ref_id', TRUE)));
		}
		else
		{
			show_404();
	...
	function replace($id = NULL)
	{
		...
		if (!$this->saisai->auth->has_permission($this->permission, 'edit') OR !$this->saisai->auth->has_permission($this->permission, 'delete'))
		{
			show_error(lang('error_no_permissions'));
		}
		...
		if (!empty($_POST))
		{
			if (!empty($_POST['saisai_replace_id']))
			{
				$replace_id = $this->input->post('saisai_replace_id');
				//$delete = is_true_val($this->input->post('saisai_delete_replacement'));
				$delete = TRUE;
				...
				{
					$this->saisai->admin->set_notification(lang('module_replaced_success'), Saisai_admin::NOTIFICATION_SUCCESS);
					$success = TRUE;
					$this->_clear_cache();
				}
			}
			...
			//redirect(saisai_uri($this->module_uri.'/edit/'.$id));
		}
		$this->load->library('form_builder');
		
		$fields = array();
		$other_options = $this->model->get_others($this->display_field, $id);
		$fields['saisai_replace_id'] = array('label' => 'Replace record:', 'type' => 'select', 'options' => $other_options, 'first_option' => 'Select record to replace...', 'style' => 'max-width: 400px', 'disabled_options' => array($id));
		//$fields['saisai_delete_replacement'] = array('label' => 'Delete replacement', 'type' => 'checkbox', 'value' => 'yes');
		if ($success)
		{
			$fields['new_saisai_replace_id'] = array('type' => 'hidden', 'value' => $replace_id);
		}
		...
		$this->saisai->admin->set_titlebar($crumbs);
		$this->saisai->admin->render('modules/module_replace', $vars);
	...
	function view($id = NULL)
	{
		if (!empty($this->preview_path) AND !empty($id))
		{
			...
			$this->saisai->admin->set_last_page($last_page);
			redirect($url);
		}
		else
		{
	...
	function refresh_field()
	{
		if (!empty($_POST))
		{
			...
			if ($fields[$field_key]['type'] == 'template')
			{
				...
				require_once(SAISAI_PATH.'libraries/Saisai_custom_fields.php');
				$saisai_cf = new Saisai_custom_fields();
				...
				$sub_fields = $saisai_cf->template($params, TRUE);
	...
	function export()
	{
		...
		if (!$this->saisai->auth->has_permission($this->permission, 'export'))
		{
			show_error(lang('error_no_permissions'));
		}
		if (!empty($_POST))
	...
	{
		if (!$this->saisai->auth->module_has_action('save') OR ($field == 'publish' AND !$this->saisai->auth->has_permission($this->permission, 'publish')))
		{
			return FALSE;
		}
	...
	{
		$fields['__saisai_module__'] = array('type' => 'hidden');
		$fields['__saisai_module__']['value'] = $this->module;
		$fields['__saisai_module__']['class'] = '__saisai_module__';

		$fields['__saisai_module_uri__'] = array('type' => 'hidden');
		$fields['__saisai_module_uri__']['value'] = $this->module_uri;
		$fields['__saisai_module_uri__']['class'] = '__saisai_module_uri__';

		$fields['__saisai_id__'] = array('type' => 'hidden');
		$fields['__saisai_id__']['value'] = (!empty($values[$this->model->key_field()])) ? $values[$this->model->key_field()] : '';
		$fields['__saisai_id__']['class'] = '__saisai_id__';
		return $fields;
	}
	...
	{
		// set tab
		if (isset($_POST['__saisai_selected_tab__']))
		{
			if (!empty($_COOKIE['saisai_tabs']))
			{
				$tab_cookie = json_decode(urldecode($_COOKIE['saisai_tabs']), TRUE);
				if (!empty($tab_cookie))
				{
					$tab_cookie[$this->module.'_edit_'.$id] = $_POST['__saisai_selected_tab__'];
	...
	protected function _process_uploads($posted = NULL)
	{
		...
		if (!empty($_FILES))
		{
			...
			// UPLOAD!!!
			if (!$this->saisai->assets->upload($params))
			{
				$errors = TRUE;
				$msg = $this->saisai->assets->last_error();
				add_error($msg);
				$this->saisai->admin->set_notification($msg, Saisai_admin::NOTIFICATION_ERROR);
			}
			else
			{
				...
				$uploaded_data = $this->saisai->assets->uploaded_data();
