...
class Installer extends Saisai_base_controller {
	...
	public function install($module = NULL)
	{
		...
		$module_file = MODULES_PATH.$module.'/libraries/Saisai_'.$module.'.php';
		if (file_exists($module_file))
		{
			$init = array('name' => $module, 'folder' => $module);
			$this->load->module_library($module, 'saisai_'.$module, $init);
			$module_lib = 'saisai_'.$module;
			if (!$this->$module_lib->install())
			{
				echo $this->saisai->installer->last_error()."\n";
			}
			else
			{
	...
	{
		if (!$this->saisai->modules->exists($module))
		{
		...
		// uninstall
		if (!$this->saisai->$module->uninstall())
		{
			echo $this->saisai->installer->last_error();
		}
		else
