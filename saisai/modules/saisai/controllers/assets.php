...
class Assets extends Module {
	...
	public function create($dir = NULL, $inline = FALSE)
	{
		...
		if (!empty($_POST))
		{
			if (!empty($_FILES['userfile___0']) AND $_FILES['userfile___0']['error'] != 4)
			{
				...
				if ($this->input->post('asset_folder')) 
				{
					...
					if (!in_array($dir, array_keys($this->saisai->assets->dirs())))
					{
						show_404();
					}
				}
				
				$subfolder = ($this->config->item('assets_allow_subfolder_creation', 'saisai')) ? str_replace('..'.DIRECTORY_SEPARATOR, '', $this->input->get_post('subfolder', TRUE)) : ''; // remove any going down the folder structure for protections
				$upload_path = $this->config->item('assets_server_path').$this->saisai->assets->dir($dir).DIRECTORY_SEPARATOR.$subfolder; //assets_server_path is in assets config
				...
				$id = $posted['file_name'];
				if ($this->saisai->assets->upload($posted))
				{
					...
					{
						$url = saisai_uri($this->module.'/inline_create/'.uri_safe_encode($dir).$query_str, FALSE);
					}
					else
					{
						$url = saisai_uri($this->module.'/create/'.uri_safe_encode($dir).$query_str, FALSE);
					}
					redirect($url);
				...
				{
					add_errors($this->saisai->assets->errors());
				}
		...
		$this->saisai->admin->set_inline(($inline === TRUE));
	...
	public function select($dir = NULL)
	{
		if (!is_numeric($dir))
		{
			$dir = saisai_uri_string(1, NULL, TRUE);
			$dirs = $this->saisai->assets->dirs();
		...
		$redirect_to = uri_safe_encode(saisai_uri(saisai_uri_string(), TRUE)); // added back to make it refresh

		$preview = ' OR <a href="'.saisai_url('assets/inline_create?asset_folder='.urlencode($dir).'&redirect_to='.$redirect_to).'" class="btn_field">Upload</a><div id="asset_preview"></div>';
	...
	{
		redirect(saisai_uri('assets/create/'.$dir));
	}
