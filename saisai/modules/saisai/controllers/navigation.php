...
class Navigation extends Module {
	...
	public function upload()
	{
		...
		$this->load->module_model(SAISAI_FOLDER, 'saisai_navigation_groups_model');
		$this->load->module_model(SAISAI_FOLDER, 'saisai_navigation_model');
		...
		if (!empty($_POST))
		{
			...
			if (!empty($_FILES['file']['name']))
			{
				...
				if (!$this->saisai->navigation->upload($params))
				{
					$error = TRUE;
				}
				...
				else
				{
					...
					$this->saisai->admin->set_notification(lang('navigation_success_upload'), Saisai_admin::NOTIFICATION_SUCCESS);
					
					redirect(saisai_url('navigation?group_id='.$params['group_id']));
				}
		...
		$fields = array();
		$nav_groups = $this->saisai_navigation_groups_model->options_list('id', 'name', array('published' => 'yes'), 'id asc');
		...
		$fields['language'] = array('type' => 'select', 'options' => $this->saisai->language->options(), 'first_option' => lang('label_select_one'));
		...
		$this->saisai->admin->render('upload', $vars, Saisai_admin::DISPLAY_NO_ACTION);
	}	
