...
/**
 * SAISAI modules advanced
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_advanced_modules
 */

// --------------------------------------------------------------------

class Saisai_advanced_module extends Saisai_base_library {
	...
	public function initialize($params = array())
	{
		...
		// the parent Saisai_base_library before the rest of the constructor'
		if ($this->has_lang())
		{
			$lang = (defined('SAISAI_ADMIN')) ? $this->saisai->auth->user_lang() : NULL;
	...
	public function __get($var)
	{
		...
		$sub_module = $this->saisai->modules->get($sub_module_name, FALSE);
		if (!empty($sub_module))
		{
			return $sub_module;
		}
	...
	public function submodules($sub = NULL)
	{
		...
		{
			$subs = array();
			$config = $this->saisai->modules->get_module_config($this->name());
			...
			{	
				...
				foreach($config as $c)
				{
					$mod = $this->saisai->modules->get($c, FALSE);
					if (!empty($mod))
					{
						$subs[$c] = $mod;
					}
	 ...
	 * @param	object	The object to attach. If blank, then it will look for a library class of 'Saisai_{$key} in the libraries folder (optional)
	 * @return	void
	 */	
	public function attach($key, $obj = NULL)
	{
		...
		else
		{
			...
			{
				// first look for a Saisai_$key library in the module's folder'
				if (file_exists($this->server_path('libraries/Saisai_'.$key.'.php')))
				{
					// this was added to prevent the loading of config files auotmatically
					$init = array('name' => $key);
					$this->load_library('saisai_'.$key, $init);
					$this->_attached[$key] =& $this->CI->{'saisai_'.$key};
				}
	...
	public function settings($item = NULL)
	{
		...
		{
			$this->_settings = $this->saisai->settings->get($this->folder());
		}
	...
	/**
	 * Returns a URL specific to the advanced module (e.g. http://localhost/saisai/{advanced_module}/create)
	 *
	 * @access	public
	 * @param	string	The URI path relative to the advanced module (optional)
	 * @return	string
	 */
	public function saisai_url($uri = '')
	{
		$uri = trim($uri, '/');
		return saisai_url($this->uri_path().'/'.$uri);
	}
	
	// --------------------------------------------------------------------
	
	/**
	 * Returns the URI path specific to the advanced module (e.g. saisai/{advanced_module})
	 *
	 * @access	public
	 * @return	string
	 */
	public function uri_path()
	{
		...
		if (empty($this->uri_path))
		{
			...
			if (file_exists($routes_file))
			{
				...
				if (isset($route))
				{
					$this->uri_path = str_replace(SAISAI_ROUTE, '', key($route));
				}
			}
		}
	...
	/**
	 * Returns the server path to the advanced module (e.g. /vars/www/httpdocs/saisai/modules/{advanced_module}/)
	 *
	 * @access	public
	...
	/**
	 * Returns the advanced modules main class name (e.g. Saisai_my_module)
	 *
	 * @access	public
	...
	/**
	 * Returns the server path to the advanced modules main class name (e.g. /vars/www/httpdocs/saisai/modules/{advanced_module}/Saisai_my_module)
	 *
	 * @access	public
	 * @return	string
	 */
	...
	public function save_cache($cache_id, $data, $group = NULL, $ttl = NULL)
	{
		...
		$this->saisai->cache->set_cache_path($orig_cache_path);
	}
	...
	public function get_cache($cache_id, $cache_group = NULL, $skip_checking = FALSE)
	{
		...
		$cached =  $this->saisai->cache->get($cache_id, $cache_group, $skip_checking);
	...
	{
		return $this->saisai->cache->clear_module($this->folder());
	}
	...
	public function is_cached($cache_id, $group = NULL)
	{
		...
		$is_cached = $this->saisai->cache->is_cached($cache_id, $group);
	...
	{
		$orig_cache_path = $this->saisai->cache->cache_path;
	...
	{
		return $this->saisai->installer->install($this->name());
	}
	...
	{
		return $this->saisai->installer->uninstall($this->name());
	}
...
// --------------------------------------------------------------------
	 ...
	 * This can be done by doing it via command line: > php index.php saisai/build/{module}.
	 * SAISAI uses > php index.php saisai/build to build optimized CSS and JS files.
	 *
	 * @access	public
	 * @return	mixed
	 */
...
/* End of file Saisai_advanced_module.php */
/* Location: ./modules/saisai/libraries/Saisai_advanced_module.php */