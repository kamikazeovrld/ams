...
/**
 * SAISAI Authorization object
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_auth
 */

// --------------------------------------------------------------------

class Saisai_auth extends Saisai_base_library {
	...
	{
		$this->CI->load->module_model(SAISAI_FOLDER, 'saisai_users_model');
		$valid_user = $this->CI->saisai_users_model->valid_user($user, $pwd);

		// check old password logins
		if (empty($valid_user))
		{
			$valid_user = $this->CI->saisai_users_model->valid_old_user($user, $pwd);
		}
		
		if (!empty($valid_user)) 
		{
			// update the hashed password & add a salt
			$salt = $this->CI->saisai_users_model->salt();
			$updated_user_profile = array('password' => $this->CI->saisai_users_model->salted_password_hash($pwd, $salt), 'salt' => $salt);
			$updated_where = array('user_name' => $user, 'active' => 'yes');


			// update salt on login
			if ($this->CI->saisai_users_model->update($updated_user_profile, $updated_where))
			{
				$this->set_valid_user($valid_user);
				$this->CI->saisai->logs->write(lang('auth_log_login_success', $valid_user['user_name'], $this->CI->input->ip_address()), 'debug');
				return TRUE;
			}
			else
			{
	...
	/**
	 * Returns the sessions namespace which helps distinguish it from other SAISAI installs (it's based on the site_name config parameter)
	 *
	 * @access	public
	 * @return	string
	 */	
	...
	{
		$key = 'saisai_'.md5(FCPATH); // unique to the site installation
		if (isset($this->CI->session))
		{
	 ...
	 * @return	string
	 */	
	public function get_saisai_trigger_cookie_name()
	{
	...
	/**
	 * Checks the SAISAI configuration for any IP restrictions
	 *
	 * @access	public
	 * @return	boolean
	 */	
	...
	{
		$restrict_ip = $this->saisai->config('restrict_to_remote_ip');
		return ($this->saisai->config('admin_enabled') AND
	...
	public function has_permission($permission, $type = '')
	{
		...
		if (!empty($user_perms))
		{
			...
			{
				$foreign_module = NULL;
				if (($permission[0] != $this->CI->module) AND in_array($permission[0], array_keys($this->CI->saisai->modules->get()))) {
	...
	{
		$this->CI->load->module_config('saisai', 'saisai', TRUE);
		$allowed = (array) $this->CI->config->item('modules_allowed', 'saisai');
	...
	public function get_permissions()
	{
		...
		$user = $CI->saisai_users_model->find_one($where);
	 ...
	 * @return	boolean
	 */	
	public function is_saisaiified()
	{
		// cache it in a static variable so we don't make multiple cookie requests
		static $is_saisaiified;
		if (is_null($is_saisaiified))
		{
			$is_saisaiified = get_cookie($this->get_saisai_trigger_cookie_name());
		}
		return $is_saisaiified;
	}
	...
	public function user_lang()
	{
		...
		if (is_null($user_lang))
		{
			...
			$cookie_val = get_cookie($this->get_saisai_trigger_cookie_name());
	...
	public function logout()
	{
		...
		$config = array(
			'name' => $this->get_saisai_trigger_cookie_name(),
			'path' => WEB_PATH
		);
		delete_cookie($config);

		// remove UI cookie
		$ui_cookie_name = 'saisai_ui_'.str_replace('saisai_', '', $this->saisai->auth->get_saisai_trigger_cookie_name());
		$config = array(
			'name' => $ui_cookie_name,
			'path' => WEB_PATH
		);
...
/* End of file Saisai_auth.php */
/* Location: ./modules/saisai/libraries/saisai/Saisai_auth.php */