 ...
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_assets
 */

// --------------------------------------------------------------------

class Saisai_assets extends Saisai_base_library {
	...
	public function __construct($params = array())
	{
		...
		$this->CI->saisai->load_model('saisai_assets');
	...
	public function initialize($params = array())
	{
		...
		$this->dir_filetypes = $this->CI->saisai->config('editable_asset_filetypes');
		$this->_dirs = list_directories($this->CI->asset->assets_server_path(), $this->CI->saisai->config('assets_excluded_dirs'), FALSE, TRUE);
	}
	...
	public function upload($params = array())
	{
		...
		foreach($_FILES as $key => $file)
		{
			if ($file['error'] == 0)
			{
				...
				// set restrictions 
				$params['max_size'] = $this->saisai->config('assets_upload_max_size');
				$params['max_width'] = $this->saisai->config('assets_upload_max_width');
				$params['max_height'] = $this->saisai->config('assets_upload_max_height');
				...
				if (!is_dir($params['upload_path']) AND $this->saisai->config('assets_allow_subfolder_creation'))
				{
				...
				else
				{
					...
					if (($this->saisai->config('set_upload_file_perms') !== FALSE) AND function_exists('chmod')
						AND is_integer($this->saisai->config('set_upload_file_perms')))
					{
						chmod($this->_data[$key]['full_path'], $this->saisai->config('set_upload_file_perms'));
					}
				}
	...
	{
		return $this->CI->saisai_assets_model->find_by_key($file);
	}
	...
	{
		return $this->CI->saisai_assets_model->delete($file);
	}
	...
	{
		return $this->CI->saisai_assets_model;
	}
	...
	{
		$editable_filetypes = $this->CI->saisai->config('editable_asset_filetypes');
	...
	{
		$excluded = array_merge($this->CI->saisai->config('assets_excluded_dirs'), $this->CI->asset->assets_folders);
		$return = array();
...
/* End of file Saisai_assets.php */
/* Location: ./modules/saisai/libraries/Saisai_assets.php */