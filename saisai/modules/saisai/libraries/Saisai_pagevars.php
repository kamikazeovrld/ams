 ...
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_pagevars
 */

// --------------------------------------------------------------------

class Saisai_pagevars extends Saisai_base_library {
	...
	/**
	 * Retrieve the SAISAI db variables for a specific location
	 *
	 * @access	public
	 * @param	boolean
	 * @return	array
	...
	public function db($parse = FALSE)
	{
		...
		$site_vars = $this->saisai->sitevars->get($location);
		
		$this->saisai->load_model('saisai_pagevariables_model');
		$this->CI->saisai_pagevariables_model->honor_page_status = $this->honor_page_status;
		$page_vars = $this->CI->saisai_pagevariables_model->find_all_by_location($location, $this->lang, $this->include_pagevar_object);
		
		// if the selected languages page variables is empty, then we try the default
		if (empty($page_vars))
		{
			$page_vars = $this->CI->saisai_pagevariables_model->find_all_by_location($location, $this->saisai->language->default_option(), $this->include_pagevar_object);
		}
...
/* End of file Saisai_pagevars.php */
/* Location: ./modules/saisai/libraries/saisai/Saisai_pagevars.php */