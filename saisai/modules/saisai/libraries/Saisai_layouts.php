...
/**
 * SAISAI layouts object
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_layouts
 */

// --------------------------------------------------------------------
// to prevent errors if 'blocks' are set in MY_saisai_layouts.php
//require_once('Saisai_blocks.php');

class Saisai_layouts extends Saisai_base_library {
	...
	public function __construct($params = array())
	{
		...
		@include(SAISAI_PATH.'config/saisai_layouts'.EXT);
		
		$this->CI->load->library('form_builder');

		if (!empty($config)) $params = $config;
		if (empty($params)) show_error('You are missing the saisai_layouts.php config file.');
	...
	public function initialize($config = array())
	{
		...
		// grab layouts from the directory if layouts auto is true in the saisai_layouts config
		...
		if (!empty($layout_files))
		{
			foreach($layout_files as $file)
			{
				...
				if (substr($group, 0, 1) != '_')
				{
					...
					{
						$this->layouts[$layout] = array('class' => 'Saisai_layout', 'group' => $group);
					}
	 ...
	 * @return	mixed 	Returns either an array of Saisai_Layout objects or a single Saisai_layout object
	 */	
	...
	{
		$default_class = 'Saisai_layout';
		$loaded_classes = array('Saisai_layout', 'Saisai_module_layout', 'Saisai_block_layout');
...
/**
 * Base SAISAI layout object.
 *
 * Can be retrieved by $this->saisai->layouts->get('{location}')
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 ...
 * @prefix		$layout->
 */
class Saisai_layout extends Saisai_base_library {
	...
	public $double_parse = NULL; // Double parse pages created in the CMS to allow for variables set in the CMS to cascade up to the layout. Valid options are TRUE/FALSE (AUTO only applies to the global SAISAI configuration)
	...
	public function is_double_parse()
	{
		...
		{
			return $this->saisai->config('double_parse');
		}
...
/**
 * Base SAISAI module layout object.
 *
 * Can be retrieved by $this->saisai->layouts->get('{location}')
 *
 * @package		SAISAI CMS
 ...
 * @prefix		$layout->
 */
class Saisai_module_layout extends Saisai_layout {
 ...
 * Base SAISAI layout object.
 *
 * Can be retrieved by $this->saisai->layouts->get('{location}', TRUE)
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 ...
 * @prefix		$layout->
 */
class Saisai_block_layout extends Saisai_layout
{
...
/* End of file Saisai_layouts.php */
/* Location: ./modules/saisai/libraries/Saisai_layouts.php */