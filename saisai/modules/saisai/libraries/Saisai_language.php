 ...
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_language
 */

// --------------------------------------------------------------------

class Saisai_language extends Saisai_base_library {
	
	public $options = array(); // The language options available. Specified in the main SAISAI config
	...
	{
		// first set the default to the values in the SAISAI config
		$_saisai_config = array('query_str_param', 'cookie_name', 'cookie_exp', 'use_cookies', 'detect_user_agent', 'default_option', 'domains');
		foreach($_saisai_config as $p)
		{
			$config = $this->saisai->config('language_'.$p);
			if (!is_null($config))
			{
				$this->$p = $config;	
			}
		...
		$this->options = $this->saisai->config('languages');
	...
	public function cookie_name()
	{
		...
		{
			return 'saisai_lang_'.substr($this->saisai->auth->get_session_namespace(), 5);
		}
	...
	function mode()
	{
		return strtolower($this->saisai->config('language_mode'));
	}
	...
	{
		return $this->saisai->set_config('language_mode', $mode);
	}
	...
	{
		$use_detect_lang = ($lang === TRUE OR (is_null($lang) AND (isset($this->CI->saisai) AND $this->CI->saisai->config('add_language_to_site_url'))));
		if (((is_string($lang) AND $lang != '') OR $use_detect_lang) AND !defined('SAISAI_ADMIN') AND USE_SAISAI_ROUTES === FALSE)
		{
			...
			if (is_null($no_lang))
			{
				$no_lang[] = SAISAI_FOLDER;
				$saisai_path = trim($this->CI->saisai->config('saisai_path'), '/');
				if (SAISAI_FOLDER != $saisai_path)
				{
					$no_lang[] = $saisai_path;
				}
			}
...
/* End of file saisai_language.php */
/* Location: ./modules/saisai/libraries/saisai_language.php */