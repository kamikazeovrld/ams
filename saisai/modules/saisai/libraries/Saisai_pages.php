 ...
 * SAISAI pages
 *
 * This class is used to find and create <a href="#saisai_page">Saisai_page</a> objects.
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_pages
 */

// --------------------------------------------------------------------

class Saisai_pages extends Saisai_base_library {
	...
	public function set_active(&$page)
	{
		...
		$this->CI->saisai_page = $page;
		$this->CI->saisai->attach('page', $page);
		$this->_active = $page;
	}
	...
	/**
	 * Returns a Saisai_page object given a provided location
	 *
	 * @access	public
	...
	public function cms()
	{
		$this->saisai->load_model('saisai_pages');
		$cms_pages = $this->CI->saisai_pages_model->list_locations(FALSE);
		return $cms_pages;
	}
	...
	{
		return $this->saisai->modules->pages();
	}
	
	// --------------------------------------------------------------------
	
	/**
	 * Finds a Saisai_page object based on either a location or ID value
	 *
	 * @access	public
	...
	public function find($id)
	{
		...
		if (!is_numeric($id))
		{
			$page = $this->CI->saisai_pages_model->find_by_location($id);
		}
		else
		{
			$page = $this->CI->saisai_pages_model->find_by_key($id);
		}
		return $page;
	}
	...
	{
		$this->saisai->load_model('saisai_pages');
		$cms_children = array_keys($this->CI->saisai_pages_model->children($root));
	...
	{
		$saisai_mode = $this->saisai->config('saisai_mode');
		if (is_array($saisai_mode))
		{
			if (isset($saisai_mode['pages']))
			{
				return $saisai_mode['pages'];
			}
			else
			{
				return 'auto';
	...
	/**
	 * Renders a Saisai_page which includes any inline editing markers.
	 * The 3rd parameter can contain Saisai_page class properties (e.g. array('render_mode' => 'cms'))
	 *
	 * @access	public
	...
	public function import($page, $sanitize = TRUE)
	{
		...
		if (!isset($this->CI->saisai_pages_model))
		{
			$this->CI->load->module_model(SAISAI_FOLDER, 'saisai_pages_model');
		}
		$model =& $this->CI->saisai_pages_model;
		...
		if (file_exists($view_twin))
		{
			...
			$layout = $this->saisai->layouts->get($pagevars['layout']);
			...
			// parse out saisai_set_var

			// for arrays... since I couldn't get it under one regex... not perfect but works OK
			$saisai_set_var_arr_regex = '#(?<!//)saisai_set_var\(([\'|"])(.+)\\1,\s*([^\)]+\s*\))\s*\)\s*;?#Um';
			//$pagevars = $this->_import_saisai_set_var_callback($saisai_set_var_arr_regex, $output, $sanitize, $pagevars);
			$output = preg_replace($saisai_set_var_arr_regex, '', $output);
	
			// for strings
			$saisai_set_var_regex = '#(?<!//)saisai_set_var\(([\'|"])(.+)\\1,\s*([^\)]+)\s*\)\s*;?#Um';
			//$pagevars = $this->_import_saisai_set_var_callback($saisai_set_var_regex, $output, $sanitize, $pagevars);
			$output = preg_replace($saisai_set_var_regex, '', $output);

			// cleanup empty tags
			$output = preg_replace('#<\?php\s*\?>#Ums', '', $output);


			// now get the variables loaded into the page by comparing the SAISAI vars after a page is rendered
			$pre_render_vars = $this->CI->load->get_vars();
			$this->saisai->pages->render($page_data['location'], array('saisaiified' => FALSE), array('render_mode' => 'views'), TRUE);
	...
	/**
	 * Helper method used for parsing out saisai_set_var
	 *
	 * @access	protected
	 * @param	string	The regex to use for matches
	 * @return	array
	 */
	protected function _import_saisai_set_var_callback($regex, $output, $sanitize, $pagevars)
	{
 ...
 * SAISAI page object.
 *
 * Can be retrieved by $this->saisai->pages->get('{location}')
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
...
class Saisai_page extends Saisai_base_library {
	...
	public static $marker_key = '__SAISAI_MARKER__'; // used for placing inline editing content in the output
	...
	protected $_saisaiified = FALSE; // is the person viewing the page logged in?... If so we will make inline editing and toolbar visible
	protected $_saisaiified_processed = FALSE; // if saisaiify has already been called
	...
	public function initialize($params = array())
	{
		...
		{
			$mod = $this->saisai->modules->get($this->view_module);
	...
	public function assign_location($location)
	{
		...
		$this->language = ($this->saisai->language->has_multiple()) ? $this->saisai->language->detect() : $this->saisai->language->default_option();

		$this->location = $location;
		
		$default_home = $this->saisai->config('default_home_view');
		...
		if ($this->saisai->language->has_multiple())
		{
			$lang_seg = (empty($segments)) ? '' : $segments[1];
			if ($this->saisai->language->lang_segment($lang_seg))
			{
				$this->saisai->language->set_selected($lang_seg);
		...
		{
			$page_data = $this->CI->saisai_pages_model->find_by_location($default_home, $this->only_published);
		...
		else 
		{
			...
			while(count($segments) >= 1)
			{
				...
				$prefix = $this->saisai->config('page_uri_prefix');
				if ($prefix)
				{
					...
					$page_data = $this->CI->saisai_pages_model->find_by_location($location, $this->only_published);
				}
				else
				{
					$page_data = $this->CI->saisai_pages_model->find_by_location($location, $this->only_published);
	...
	public function assign_variables($views_path = NULL, $page_mode = NULL)
	{
		...
		$page_mode = (empty($page_mode)) ? $this->saisai->pages->mode() : $page_mode;
		...
		$this->saisai->pagevars->initialize($init_vars);
		$vars = $this->saisai->pagevars->retrieve($this->location, $page_mode);
	 ...
	 *
	 * @access	public
	 * @param	mixed	Can be either a name of the layout or a Saisai_layout object
	 * @return	void
	 */	
	public function assign_layout($layout)
	{
		...
		{
			$layout = $this->saisai->layouts->get($layout);
		}
		
		if (is_a($layout, 'Saisai_layout'))
		{
			$this->layout = $layout;
	 ...
	 * @return	string
	 */	
	public function render($return = FALSE, $saisaiify = TRUE)
	{
		// check the _page_data to see if it even exists in the CMS
		if (!isset($this->_page_data['id']))
		{
			$this->render_mode = 'views';
			return $this->variables_render($return, $saisaiify);
		}
		else
		{
			$this->render_mode = 'cms';
			return $this->cms_render($return, $saisaiify);
		}
	}
	 ...
	 * @return	string
	 */
	public function cms_render($return = FALSE, $saisaiify = FALSE)
	{
		...
		if (!empty($this->layout))
		{
			...
			if ($saisaiify) $output = $this->saisaiify($output);
	 ...
	 * @return	string
	 */
	public function variables_render($return = FALSE, $saisaiify = FALSE)
	{
		...
		if (!empty($this->language) AND !$this->saisai->language->is_default($this->language))
		{
		...
		if (file_exists($check_file))
		{
			...
			{
				$layout = $vars['layout'];
				$this->layout = $this->saisai->layouts->get($layout);
			}
			...
			{
				$vars['body'] = $body;
				$layout_dir = trim($this->saisai->layouts->layouts_folder, '/'); // remove any trailing slash... we'll add it below'
	 ...
	 * @return	string
	 */
	public function saisaiify($output)
	{
		// if not logged in then we remove the markers
		if (!$this->saisai->config('admin_enabled') OR $this->variables('saisaiified') === FALSE OR
			!$this->_saisaiified OR empty($output) OR (defined('SAISAIIFY') AND SAISAIIFY === FALSE) )
		{
		...
		// add top edit bar for saisai
		$this->CI->config->module_load('saisai', 'saisai', TRUE);
		
		// render the markers to the proper html
		$output = $this->render_all_markers($output);
		
		// set main image and assets path before switching to saisai assets path
		...
		$this->CI->asset->assets_path = $this->saisai->config('saisai_assets_path');
		$this->CI->load->helper('ajax');
		$this->CI->load->library('form');
		$last_page = uri_path();
		if (empty($last_page)) $last_page = $this->saisai->config('default_home_view');
		$vars['last_page'] = uri_safe_encode($last_page);

		if (!$this->_saisaiified_processed)
		{
			// create the inline edit toolbar
			$inline_edit_bar = $this->saisai->admin->toolbar();
			$saisai_js_obj = "<script>if (typeof saisai == 'undefined') saisai = {}</script>\n";
			$inline_css = css('saisai_inline', 'saisai', array('output' => $this->saisai->config('saisai_assets_output')));
			$output = preg_replace('#(</head>)#i', $saisai_js_obj.$inline_css."\n$1", $output);
			$output = preg_replace('#(</body>)#i', $inline_edit_bar."\n$1", $output);
			$this->CI->config->set_item('assets_path', $this->CI->config->item('assets_path'));
		}
		$this->_saisaiified_processed = TRUE;
	...
	public function render_marker($key)
	{
		...
		if ($this->saisai->config('admin_enabled') AND
			is_saisaiified() AND
			(is_null($this->CI->load->get_var('saisaiified')) OR $this->CI->load->get_var('saisaiified') === TRUE)
			AND $this->CI->saisai->auth->has_permission($perm, 'edit')
			)
		{
			if (empty($label))
			{
			...
			$output = '<span class="__saisai_marker__" data-href="'.saisai_url($module).'/'.$edit_method.'/" data-rel="'.$id.'" title="'.$label.'" data-module="'.$module.'" data-published="'.$published.'"';
	...
	public function render_all_markers($output)
	{
		...
		{	
			...
			//$output = preg_replace('#(=["\'][^<]*)('.$marker_reg_ex.')#Ums', '${2}${1}', $output);  // doesn't work with saisai_var in multiple tag attributes
	...
	/**
	 * Returns the Saisai_page properties which includes location, layout, published and cached values
	 *
	 * @access	public
	 ...
	 * The SAISAI config value of "max_page_params" must have a value greater then 0 (which is the default) for parameters to be passed.
	...
	{
		$this->saisai->load_model('saisai_pages');
		$this->saisai->load_model('saisai_pagevariables');
		
		$page_props = $this->CI->saisai_pages_model->create();
		$page_props->location = $this->location;
		if (empty($this->layout))
		{
			$layout = new Saisai_layout();
		}
		else if (is_string($this->layout))
		{
			$layout = $this->saisai->layouts->get($this->layout);
		}
		else
		{
			$layout = $this->layout;
		...
		{
			$page_var = $this->CI->saisai_pagevariables_model->create();
			$page_var->page_id = $id;
			$page_var->name = $key;
			if (is_array($val))
			{
	 ...
	 * The Saisai configuration value of "auto_search_views" must be set to TRUE or a number greater than 1 (it is FALSE by default).
	...
	public function find_view_file($view, $depth = NULL)
	{
		if (is_null($depth))
		{
			$depth = (is_int($this->saisai->config('auto_search_views'))) ? $this->saisai->config('auto_search_views') : $this->get_max_page_param(); // if not a number (e.g. set to TRUE), we default to 2
		}
		
		if (!$this->saisai->config('auto_search_views') AND empty($depth)) return NULL;
	...
	public function get_max_page_param()
	{
		...
		if (is_array($this->saisai->config('max_page_params')))
		{
			...
			foreach($this->saisai->config('max_page_params') as $key => $val)
			{
		...
		{
			$max_page_params = (int)$this->saisai->config('max_page_params');
		}
		return $max_page_params;
	}
}

/* End of file Saisai_pages.php */
/* Location: ./modules/saisai/libraries/saisai/Saisai_pages.php */