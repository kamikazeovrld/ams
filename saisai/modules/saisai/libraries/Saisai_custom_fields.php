...
/**
 * SAISAI custom fields object
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/library/form_builder
 * @autodoc		FALSE
 */

// --------------------------------------------------------------------

class Saisai_custom_fields {
	...
	public function wysiwyg($params)
	{
		...
		$js = '<script type="text/javascript">
			myMarkItUpSettings.previewParserPath = "'.saisai_url().'/preview";
		</script>';
	...
	public function asset($params)
	{
		...
		$editable_filetypes = $this->saisai->config('editable_asset_filetypes');
	...
	public function inline_edit($params)
	{
		...
		else
		{
			if (!empty($params['module']))
			{
				...
				$modules = $this->CI->saisai->modules->get(NULL, FALSE);
				...
				{
					$CI =& get_instance();
					$module = $CI->saisai->modules->get($params['module'], FALSE);
				...
				if ($this->saisai->auth->has_permission($permission))
				{
	...
	public function multi($params)
	{
		...
		if (!empty($params['module']))
		{
			...
			if (strpos($params['module'], '/') === FALSE)
			{
				$module = $this->CI->saisai->modules->get($params['module'], FALSE);
				$uri = (!empty($module)) ? $module->info('module_uri') : '';
			}
			else
			{
			...
			// check for modules with saisai_ prefix
			$permission = (!empty($module)) ? $module->permission : $uri;
			if (!empty($params['module']) AND $this->saisai->auth->has_permission($permission))
			{
				$inline_class = 'add_edit '.$uri;
				$params['class'] = (!empty($params['class'])) ? $params['class'].' '.$inline_class : $inline_class;
				$params['data'] = array(
	...
	/**
	 * Creates a dropdown select of languages identified in the MY_saisai.php file
	 *
	 * @access	public
	 * @param	array Fields parameters
	 * @return	string
	...
	public function language($params)
	{
		...
		{
			$params['type'] = 'select';
			$params['options'] = $this->CI->saisai->language->options();
		}
		return $form_builder->create_select($params);
	...
	public function block($params)
	{
		...
		{
			// check to make sure there is no conflict between page columns and layout vars
			$layout = $this->saisai->layouts->get($params['block_name'], 'block');
		...
		else
		{
			...
			if (!isset($params['options']))
			{
				...
				if (isset($params['folder']))
				{
					...
					$params['options'] = $this->CI->saisai->blocks->options_list($params['where'], $params['folder'], $params['filter'], $params['order'], $params['recursive']);
				}

				// otherwise we use the ones found on the MY_saisai_layouts.php
				else
				{
					...
					$params['options'] = $this->CI->saisai->layouts->options_list(TRUE, $params['group']);
				}
			}
...
/* End of file Saisai_custom_fields.php */
/* Location: ./modules/saisai/libraries/Saisai_custom_fields.php */