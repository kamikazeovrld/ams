...
/**
 * SAISAI navigation object
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_navigation
 */

// --------------------------------------------------------------------

class Saisai_navigation extends Saisai_module {
	 ...
	 *
	 * The <a href="[user_guide_url]helpers/saisai_helper">saisai_nav helper</a> function is an alias to this method.
	 *
	<ul>
	...
	public function render($params = array())
	{
		...
		// if ($this->saisai->language->has_multiple() AND empty($p['language']))
		// {
		// 	$p['language'] = (!empty($p['language'])) ? $p['language'] : $this->saisai->language->detect();
		// }
		if ($p['cache'] === TRUE)
		{
			// cache id and group
			$cache_id = md5(json_encode($params));
			$cache_group = $this->CI->saisai->config('page_cache_group');
			$cache = $this->CI->saisai->cache->get($cache_id, $cache_group);
			if (!empty($cache))
			{
				return $cache;
			}
		...
		if (empty($p['items']))
		{
			// get the menu data based on the SAISAI mode or if the file parameter is specified use that
			if ($this->CI->saisai->navigation->mode() == 'views' OR !empty($params['file']))
			{
			...
			{
				if ($this->CI->saisai->navigation->mode() != 'cms')
				{
				...
				if (!empty($params['include_default_language']))
				{
					...
					$default_lang = $this->saisai->language->default_option();
		...
		if ($p['cache'] === TRUE)
		{
			$this->CI->saisai->cache->save($cache_id, $return, $cache_group, $this->CI->saisai->config('page_cache_ttl'));
		}
		return $return;
	}
	...
	/**
	 * Uploads a static'navigation structure which is most like the <span class="file">saisai/application/views/_variables/nav.php</span> file
	 *
	 * @access	public
	 * @param	array	config preferences (optional)
	 * @return	boolean
	...
	public function upload($params = array())
	{
		...
		if (!empty($$var))
		{
			...
			foreach($nav as $key => $item)
			{
				...
				$save['selected'] = (!empty($item['selected'])) ? $item['selected'] : $item['active']; // must be different because "active" has special meaning in SAISAI
	...
	public function groups()
	{
		...
		return $this->CI->saisai_navigation_groups_model->find_all();
	}
	...
	public function group($group)
	{
		...
		if (is_numeric($group))
		{
			$group = $this->CI->saisai_navigation_groups_model->find_by_key($group);
		}
		else
		{
			$group = $this->CI->saisai_navigation_groups_model->find_one(array('name' => $group));
		}
		return $group;
	}
	...
	public function mode()
	{
		...
		{
			if (isset($saisai_mode['navigation']))
			{
				return $saisai_mode['navigation'];
			}
			else
			{
				return 'auto';
	...
	{
		if (!isset($this->CI->saisai_navigation_groups_model))
		{
			$this->saisai->load_model('saisai_navigation_groups');
		}
	}
}

/* End of file Saisai_navigation.php */
/* Location: ./modules/saisai/libraries/Saisai_navigation.php */