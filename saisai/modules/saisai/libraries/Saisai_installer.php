...
/**
 * SAISAI install object
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_installer
 */

// --------------------------------------------------------------------

class Saisai_installer extends Saisai_base_library {
	...
	/**
	 * Creates the permissions in SAISAI necessary for the advanced module
	 *
	 * @access	public
	 * @return	void
	 */	
			...
			if (is_array($permissions))
			{
				...
				foreach($permissions as $key => $val)
				{
					if (!empty($val))
					{
						if (is_int($key))
						{
							$this->saisai->permissions->create_simple_module_permissions($val);
							// $save[$i]['name'] = $val;
				...
				if (!$this->saisai->permissions->save($save))
				{
					return FALSE;
				}
			}
			...
			if (is_array($this->config['permissions']))
			{
				foreach($this->config['permissions'] as $key => $val)
				{
					if (!empty($val))
					{
						if (is_int($key))
						{
							$this->saisai->permissions->delete_simple_module_permissions($val);
						}
						else
						{
							$where['name'] = $key;
							$this->saisai->permissions->delete($where);
						}
					}
				}
			}
	...
	/**
	 * Adds the advanced module to the MY_saisai.php "modules_allowed" config array
	 *
	 * @access	public
	 * @return	void
	 */	
	public function allow()
	{
		...
		// add to modules_allowed to MY_saisai and to the database
		if (!in_array($module, $this->saisai->config('modules_allowed')))
		{
			// add to advanced module config
			$my_saisai_path = APPPATH.'config/MY_saisai.php';

			$modules_allowed = $this->saisai->config('modules_allowed');
			$modules_allowed[] = $module;

			// write allowed modules to MY_saisai.php
	...
	public function disallow()
	{
		...
		// remove to modules_allowed to MY_saisai and to the database
		if (in_array($module, $this->saisai->config('modules_allowed')))
		{
	 ...
	 *
	 * @access	protected
	 * @param	array 	Allowed modules to save in MY_saisai.php
	 * @return	void
	 */	
	protected function _write_allowed_modules($allowed)
	{
		...
		$content = file_get_contents($my_saisai_path);
		...
		if (!write_file($my_saisai_path, $content))
		{
			$this->_add_error(lang('error_could_not_create_file', $my_saisai_path));
		}
	}
	...
	{
		// save to database if the settings is there
		$module_obj = $this->saisai->modules->get($module);
		if (!empty($module_obj) AND $this->saisai->modules->is_advanced($module_obj))
		{
			$settings = $module_obj->settings_fields();
			if (isset($settings['modules_allowed']))
			{
				$this->saisai->settings->save(SAISAI_FOLDER, 'modules_allowed', $allowed);
			}
		}
	}
	...
	/**
	 * Checks that the module is comptable with the installed version of SAISAI
	 *
	 * @access	public
	 * @return	boolean
	 */	
	public function is_compatible()
	{
		if (isset($this->config['compatibility']))
		{
			...
			$saisai_version = $this->saisai->version();

			// if the current version of SAISAI is greater then or equal to the compatability version, the we are good to go
			if (version_compare($compatibility, $saisai_version, '>='))
			{
				return FALSE;
			}
			return TRUE;
...
/* End of file Saisai_installer.php */
/* Location: ./modules/saisai/libraries/Saisai_installer.php */