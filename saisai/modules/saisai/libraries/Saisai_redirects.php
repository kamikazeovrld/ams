 ...
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_redirects
 */

// --------------------------------------------------------------------

class Saisai_redirects extends Saisai_base_library {
	...
	public $aggressive_redirects = array(); // The pages to redirect to regardless if it's found by SAISAI. WARNING: Run on every request.
	public $passive_redirects = array(); // The pages to redirect to only AFTER no page is found by SAISAI
	...
	const REDIRECT_COUNT = '__SAISAI_REDIRECT_COUNT__';
	const REDIRECT_ORIG = '__SAISAI_REDIRECT_ORIG__';
	...
	public function execute($show_404 = TRUE, $only_passive = TRUE)
	{
		...
		if ($show_404 === TRUE)
		{
			...
			{
				$error_404 = $this->saisai->pages->render('404_error', array(), array('render_mode' => 'cms'), TRUE);
			}

			// then views
			if (empty($error_404) AND file_exists(APPPATH.'views/404_error'.EXT))
			{
				$error_404 = $this->saisai->pages->render('404_error', array(), array('render_mode' => 'views'), TRUE);
			}
...
/* End of file Saisai_redirects.php */
/* Location: ./modules/saisai/libraries/Saisai_redirects.php */