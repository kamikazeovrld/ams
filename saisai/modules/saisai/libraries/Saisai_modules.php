...
/**
 * SAISAI modules object
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/modules/saisai/saisai_modules
 */

// --------------------------------------------------------------------

require_once('Saisai_installer.php');

class Saisai_modules extends Saisai_base_library {
	...
	static public function get_all_module_configs()
	{
		...
		//require_once(MODULES_PATH.SAISAI_FOLDER.'/libraries/saisai_modules.php');
		$SAISAI = SAISAI();
		$allowed = $SAISAI->config('modules_allowed');
		$module_init = array();
		...
		$saisai_module_init = (array)self::get_module_config('saisai');
		self::$_modules_grouped['saisai'] = $module_init;
		$module_init = array_merge($my_module_init, $saisai_module_init);
		
		// no longer need these so we get rid of them
		unset($my_module_init, $saisai_module_init);
	...
	static public function get_module_config($module)
	{
		...
		{
			case 'saisai':
				$file_path = SAISAI_PATH.'/config/saisai_modules.php';
				break;
			case 'application': case 'app':
				$file_path = APPPATH.'/config/MY_saisai_modules.php';
				break;
			default:
				$file_path = MODULES_PATH.$module.'/config/'.$module.'_saisai_modules.php';
		}
	...
	{
		// check for specific module overwrites like for Saisai_navigation and Saisai_block
		if (isset($init['folder']))
		{
			$class_name = 'Saisai_'.strtolower($mod);
			$file_path = MODULES_PATH.$init['folder'].'/libraries/'.$class_name.EXT;

			if (file_exists($file_path))
			{
				// class must extend the saisai_module class to be legit
				if (strtolower(get_parent_class($class_name)) == 'saisai_module')
				{
					$this->CI->load->module_library($init['folder'], strtolower($class_name));
					$saisai_module =& $this->CI->$class_name;
				}
			}
		}
		
		if (!isset($saisai_module))
		{
			$saisai_module = new Saisai_module();
		}
		$saisai_module->initialize($mod, $init);
		$this->_modules[$mod] = $saisai_module;
	}
	 ...
	 * @return	object	Saisai_module object
	 */	
	...
	{
		return is_a($module, 'Saisai_advanced_module');
	}
	 ...
	 *
	 * @access	public
	 * @param	boolean	Determines whether to include the "saisai" module with the return value
	 * @return	array	An array of Saisai_advanced_module objects
	 */	
	public function advanced($include_saisai = FALSE)
	{
		$advanced = array();
		if ($include_saisai)
		{
			$advanced['saisai'] =& $this->saisai;
		}
		foreach($this->saisai->config('modules_allowed') as $module)
		{
			$advanced[$module] =& $this->saisai->$module;
		}
		return $advanced;
	}
	...
	{
		$allowed = $this->saisai->config('modules_allowed');
		$allowed[] = 'saisai';
	...
	public function install($module)
	{
		...
		$key = Saisai_install::INSTALLED_SETTINGS_KEY;
		
		$installed = $this->saisai->settings->get($module->name(), $key);
		if (empty($installed))
		{
			$installed = array();
		}
		$installed[$module->name()] = TRUE;
		$this->saisai->settings->save($module->name(), $key, $installed);
...
/**
 * SAISAI module object.
 *
 * Can be retrieved by $this->saisai->modules->get('{module_name}')
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 ...
 * @prefix		$module->
 */
class Saisai_module extends Saisai_base_library {
	...
	{	
		if (empty($this->_init))
		{
 			$inits = Saisai_modules::get_all_module_configs();
		...
		if (empty($this->_info))
		{
			...
			$defaults = array(
				...
				'js_controller' => 'saisai.controller.BaseSaisaiController',
				'js_controller_path' => '',
			...
			{
				//$info['js_controller'] = 'saisai.controller.'.$info['js_controller'];
			...
			if (empty($info['js_controller_path']))
			{
				$info['js_controller_path'] = js_path('', SAISAI_FOLDER);
			}
...
/* End of file Saisai_modules.php */
/* Location: ./modules/saisai/libraries/saisai/Saisai_modules.php */