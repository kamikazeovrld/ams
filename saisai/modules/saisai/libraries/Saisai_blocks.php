...
/**
 * SAISAI blocks object
 *
 * @package		SAISAI CMS
 * @subpackage	Libraries
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_blocks
 */

// --------------------------------------------------------------------
class Saisai_blocks extends Saisai_module {
	 ...
	 *
	 * The <a href="[user_guide_url]helpers/saisai_helper">saisai_block helper</a> function is an alias to this method.
	 *
	<ul>
	...
	* @access	public
	...
	public function render($params, $vars = array(), $check_db = TRUE, $scope = NULL)
	{
		...
		if (!empty($p['model']))
		{
			$data = saisai_model($p['model'], $p);
			$module = $this->CI->saisai->modules->get($p['model']);
			if ($module)
			{
		...
		else if (!empty($p['view']))
		{
			...
			{
				$language = (!empty($p['language'])) ? $p['language'] : $this->saisai->language->detect();
			}
			else
			{
				$language = $this->saisai->language->default_option();
			}

			// test that the file exists in the associated language
			if (!empty($language) AND !$this->saisai->language->is_default($language))
			{
			...
			// only check database if the saisai_mode does NOT equal 'views, the "only_views" parameter is set to FALSE and the view name does not begin with an underscore'
			if ($check_db AND (($p['mode'] == 'auto' AND $this->mode() != 'views') OR $p['mode'] == 'cms') AND substr($p['view'], 0, 1) != '_')
			{
				$this->saisai->load_model('saisai_blocks');

				// find the block in SAISAI db
				$block = $this->CI->saisai_blocks_model->find_one_by_name_and_language($p['view'], $language);
				...
				{
					$block = $this->CI->saisai_blocks_model->find_one_by_name($p['view']);
				}
				...
				if (isset($block->id))
				{
					...
					if ($p['editable'] === TRUE)
					{
						$view = saisai_edit($block->id, 'Edit Block: '.$block->name, 'blocks').$view;
					}
				}
		...
		if ($p['cache'] === TRUE)
		{
			$this->CI->cache->save($cache_id, $output, $cache_group, $this->CI->saisai->config('page_cache_ttl'));
		}
	...
	public function mode()
	{
		...
		{
			if (isset($saisai_mode['blocks']))
			{
				return $saisai_mode['blocks'];
			}
			else
			{
				return 'auto';
...
/* End of file Saisai_blocks.php */
/* Location: ./modules/saisai/libraries/Saisai_blocks.php */