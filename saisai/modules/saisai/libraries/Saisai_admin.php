 ...
 * @category	Libraries
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/libraries/saisai_admin
 */

// --------------------------------------------------------------------

class Saisai_admin extends Saisai_base_library {
	
	protected $main_layout = 'admin_main'; // The main layout file for the SAISAI admin
	...
	public function __construct($params = array())
	{
		...
		//$this->CI->load->module_helper(SAISAI_FOLDER, 'saisai');... alternative syntax
		$this->saisai->load_helper('saisai');
	}
	
	// --------------------------------------------------------------------
	
	/**
	 * Initialize the SAISAI admin object
	 *
	...
	public function initialize($params = array())
	{
		...
		// load saisai helper
		$this->CI->load->module_helper(SAISAI_FOLDER, 'saisai');
		...
		if (!$this->saisai->auth->can_access() AND $this->validate)
		{
			show_404();
		}
		
		// set asset output settings
		$this->CI->asset->assets_output = $this->saisai->config('saisai_assets_output');
		...
		$load_vars = array(
			...
			'nav' => $this->nav(),
			'modules_allowed' => $this->saisai->config('modules_allowed'),
		...
		if ($this->validate)
		{
			...
			$load_vars['session_key'] = $this->saisai->auth->get_session_namespace();
		}
		$this->CI->js_controller_path = js_path('', SAISAI_FOLDER);
		...
		$this->main_layout = $this->saisai->config('main_layout');
		
		$this->set_inline($this->CI->input->get('inline'));
		
		// set last page
		$this->last_page = $this->saisai->auth->user_data('saisai_last_page');
	 ...
	 * @param	string	The display mode to use (can be the class constant Saisai_admin::DISPLAY_NO_ACTION, DISPLAY_COMPACT, DISPLAY_COMPACT_NO_ACTION, DISPLAY_COMPACT_TITLEBAR, DISPLAY_DEFAULT) (optional)
	...
	public function render($view, $vars = array(), $mode = '', $module = NULL)
	{
		...
		if (empty($vars['notifications']))
		{
			...
			$vars['notifications'] = $this->CI->load->module_view(SAISAI_FOLDER, '_blocks/notifications', $vars, TRUE);
		}
		...
		// set inline
		if (!empty($_POST['saisai_inline']) OR (int)$this->CI->input->get('inline') != 0)
		{
			if (!empty($_POST['saisai_inline']) AND $_POST['saisai_inline'] != 0)
			{
				$inline = $this->CI->input->post('saisai_inline');
			}
		...
		{
			$vars['form_action'] = saisai_url($vars['form_action']);
		}
		...
		if (!empty($layout))
		{
			...
			{
				$this->CI->load->module_view(SAISAI_FOLDER, '_layouts/'.$layout, $vars);
			}
		}
		else
		{
	...
	/**
	 * Checks that the user is logged in to SAISAI and if not will redirect to the login
	 *
	 * @access	public
	 * @return	void
	 */	
	public function check_login()
	{
		...
		// check if logged in
		if (!$this->CI->saisai->auth->is_logged_in() OR !is_saisaiified())
		{
			$login = $this->CI->saisai->config('saisai_path').'login';
			
			// logout officially to unset the cookie data
			$this->CI->saisai->auth->logout();
	...
	public function set_last_page($page = NULL)
	{
		...
		{
			$this->saisai->auth->set_user_data('saisai_last_page', $page);
		}
	}
	...
	public function add_recent_page($link, $name, $type)
	{
		...
		if (count($user_data['recent']) > $this->saisai->config('max_recent_pages'))
		{
	...
	public function nav()
	{
		$nav = $this->saisai->config('nav');
		
		$nav_orig = $nav;
		
		$modules = array('saisai');
		$modules = array_merge($modules, $this->saisai->config('modules_allowed'));
		...
		foreach($modules as $module)
		{
			...
			if (file_exists($nav_path))
			{
				...
				{
					foreach ($config['nav'] as $section => $saisai_modules)
					{
						if (is_array($saisai_modules) AND ! empty($saisai_modules))
						{
							foreach ($saisai_modules as $saisai_module => $saisai_module_title)
							{
								if (array_key_exists($saisai_module, $config['module_overwrites'])
											AND 
											((array_key_exists('hidden', $config['module_overwrites'][$saisai_module]) AND
												($config['module_overwrites'][$saisai_module]['hidden']))
											OR 
											(array_key_exists('disabled', $config['module_overwrites'][$saisai_module]) AND
												($config['module_overwrites'][$saisai_module]['disabled']))

											))
								{
									unset($config['nav'][$section][$saisai_module]);
								}
							}
						}
					}
		...
		if ($this->saisai->config('nav_auto_arrange'))
		{
			// rearrange
			$orig_nav = $nav;
	...
	public function nav_selected()
	{
		...
		{
			if (saisai_uri_segment(1) == '')
			{
				$nav_selected = 'dashboard';
			}
			else
			{
				$nav_selected = saisai_uri_segment(1);
			}
		}
		else
		{
	...
	/**
	 * Returns an array of CSS files for the admin including SAISAI "xtra_css" parameter
	 *
	 * @access	public
	 * @return	array
	 */	
	...
	{
		$modules = $this->saisai->config('modules_allowed');
		...
		if ($this->saisai->config('xtra_css'))
		{
			$css[] = array('' => $this->saisai->config('xtra_css'));
		}
		return $css;
	...
	public function set_language($language)
	{
		...
		$this->CI->saisai->auth->set_user_data('language', $language);
	}
	...
	{
		// set the language based on first the users profile and then what is in the config... (FYI... saisai_auth is loaded in the hooks)
		$language = $this->saisai->auth->user_data('language');
	...
	public function load_languages()
	{
		...
		// load this language file first because saisai_modules needs it
		$this->CI->load->module_language(SAISAI_FOLDER, 'saisai', $language);
		$modules = $this->saisai->config('modules_allowed');
		foreach($modules as $module)
		{
	...
	public function load_js_localized($js_localized = array(), $load = TRUE)
	{
		...
		if (empty($localized))
		{
			$localized = json_lang('saisai/saisai_js', FALSE);
		}
	...
	/**
	 * Sets the Saisai admin page title based on the module name if set or uri segments
	 *
	 * @access	public
	...
	{
		$segs = (array) $segs;
		$simple_module_configs = $this->saisai->modules->get_module_config('app');
		$page_title = lang('saisai_page_title').' : ';
	...
	{
		if ($this->saisai->config('saved_page_state_max') == 0)
		{
			return FALSE;
		}
		$state_key = $this->get_state_key();
		if (!empty($state_key))
		{
			...
			if (count($user_data['page_state']) > $this->saisai->config('saved_page_state_max'))
			{
	...
	{
		$cookie_val = json_decode(urldecode($this->CI->input->cookie('saisai_ui')), TRUE);
		if (!empty($key))
		{
	...
	public function set_ui_cookie($key = NULL, $val = NULL)
	{
		...
		$this->CI->input->set_cookie('saisai_ui', $cookie_val);
	}
	...
	{
		if (empty($type)) $type = Saisai_admin::NOTIFICATION_SUCCESS;
	...
	public function dashboards()
	{
		...
		if (!empty($dashboards_config))
		{
			...
			else if (is_array($dashboards_config))
			{
				...
				{
					if ($module == 'saisai' OR $this->saisai->auth->has_permission($module))
					{
						$dashboards[] = $module;
					}
				}
	...
	/**
	 * Adds a dashboard to the SAISAI configuration settings which will be displayed on the dashboard page
	 *
	 * @access	public
	...
	{
		$dashboards = $this->saisai->config('dashboards');
		...
		$this->saisai->set_config('dashboards', $dashboards);
	}
	...
	public function toolbar_tools()
	{
		...
		foreach($modules as $module)
		{
			// check if there is a dashboard controller for each module
			$t = $module->tools();
			if ($t AND $this->CI->saisai->auth->has_permission($module->name()))
			{
				$tools = $tools + $t;
			}
		}
	...
	{
		$user_lang = $this->saisai->auth->user_lang();
		$this->saisai->load_language('saisai_inline_edit', $user_lang);
		$this->saisai->load_language('saisai_js', $user_lang);
	
		$vars['page'] = $this->saisai->page->properties();
		$vars['layouts'] = $this->saisai->layouts->options_list();
		$vars['language'] = $this->saisai->language->detect();
		$vars['language_mode'] = $this->saisai->language->mode();
		$vars['language_default'] = $this->saisai->language->default_option();
		$vars['tools'] = $this->toolbar_tools();
		$vars['js_localized'] = json_lang('saisai/saisai_js', $user_lang);
		$vars['is_saisaiified'] = is_saisaiified();
		$vars['can_edit_pages'] = $this->CI->saisai->auth->has_permission('pages', 'edit');
		...
		else
		{
			...
			$vars['others'] = $this->CI->saisai_pages_model->get_others('location', $location, 'location');
		}
		...
		$vars['init_params']['cookiePath'] = $this->CI->saisai->config('saisai_cookie_path');
		$vars['init_params']['imgPath'] = img_path('', 'saisai');
		$vars['init_params']['cssPath'] = css_path('', 'saisai');
		$vars['init_params']['jsPath'] = js_path('', 'saisai');
		$vars['init_params']['editor'] = $this->saisai->config('text_editor');
		$vars['init_params']['editorConfig'] = $this->saisai->config('ck_editor_settings');
		$last_page = uri_path();
		if (empty($last_page)) $last_page = $this->saisai->config('default_home_view');
		$vars['last_page'] = uri_safe_encode($last_page);
		
		$output = $this->CI->load->module_view(SAISAI_FOLDER, '_blocks/inline_edit_bar', $vars, TRUE);
		
		return $output;
	}
}

/* End of file Saisai_admin.php */
/* Location: ./modules/saisai/libraries/Saisai_admin.php */