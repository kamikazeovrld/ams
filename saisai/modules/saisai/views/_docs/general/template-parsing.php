...
<h2>Overview</h2>
<p>By default, SAISAI disables PHP code from being saved in your module data
(you must change the <a href="<?=user_guide_url('modules/simple')?>">sanitize_input</a> setting for the module).
SAISAI has implemented the <a href="http://dwoo.org/" target="_blank">Dwoo</a> PHP 5 based templating system.
</p>

<h3>MY_Parser</h3>
<p>SAISAI overwrites the default CodeIgniter Parser library with the <a href="<?=user_guide_url('libraries/my_parser')?>">MY_Parser</a>
library to implement the expanded Dwoo templating syntax.</p>


<h3>String Helper Functions</h3>
<p>SAISAI comes with the following <a href="<?=user_guide_url('helpers/my_string_helper')?>">string helper functions</a> to help with parsing and converting from the Dwoo templating syntax:</p>
<ul>
...
<ul>
	...
	<li>{saisai_var} - Maps to the <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_var')?>">saisai_var()</a> function.</li>
	<li>{saisai_model('module_name', array(key="val"...)} - Maps to the <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_model')?>">saisai_model()</a> function. The first parameter is usually the name of your model and does not need to include the "_model" suffix (usually the same as the module name).</li>
	<li>{saisai_block(array(key="val"...))} - Maps to the <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_block')?>">saisai_block()</a> function.</li>
	<li>{saisai_nav(array(key="val"...))} - Maps to the <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_nav')?>">saisai_nav()</a> function.</li>
	<li>{saisai_edit(id, label, module, xOffset, yOffset)} - Maps to the <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_edit')?>">saisai_edit()</a> function.</li>
</ul>
...
<pre class="brush:php">
... 
{$mydata = saisai_model('users', find="all")}
...
<p>SAISAI also allows for php like conditional statements to be inserted into the view using <dfn>{if ... }</dfn>, <dfn>{elseif ...}</dfn> and <dfn>{/if}</dfn>.
For example:
</p>
...
<h2>Examples</h2>
<p>Below are examples of SAISAI's supported parsing syntax. For even more information, visit the <a href="http://wiki.dwoo.org/index.php/Main_Page" target="_blank">Dwoo Wiki</a></p>
...
<pre class="brush:php">
$this->load->model('saisai_users_model');
$data['users'] = $this->saisai_users_model->find_by_key(1);
...
<pre class="brush:php">
$this->load->model('saisai_users_model');
$data['users'] = $this->saisai_users_model->find_all();
$str = '
{loop $users}
