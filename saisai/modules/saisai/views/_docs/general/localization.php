...
<p>SAISAI CMS 1.0 now provides basic multi-language support through the CMS for pages and modules. You can configure the number of
supported languages in the <a href="<?=user_guide_url('installation/configuration')?>">SAISAI config</a>
by adding to the <dfn>$config['languages']</dfn> parameter. The <a href="<?=user_guide_url('libraries/saisai_language')?>">Saisai_language</a> class
...
<h3>CMS Pages</h3>
<p>When multiple languages are configured in the <a href="<?=user_guide_url('installation/configuration')?>">SAISAI config</a>, you will see an additional drop down when editing or creating a page in the CMS.
...
<p>To use the segment method, set the <dfn>language_mode</dfn> configuration parameter to <dfn>segment</dfn> or <dfn>both</dfn> in the MY_saisai.php file like so:</p>
<pre class="brush:php">
...
<p>To use the query string method, set the <dfn>language_mode</dfn> configuration parameter to <dfn>query_string</dfn> or <dfn>both</dfn> in the MY_saisai.php file like so:</p>
<pre class="brush:php">
...
<pre class="brush:php">
$this->saisai->language->set_selected('es');
</pre>
...
<h3>View Pages</h3>
<p>Additionally, if your pages are using views  and there is a language value set by the user using <a href="<?=user_guide_url('libraries/saisai_language')?>">Saisai_language</a>,
...
the <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_block')?>">saisai_blocks</a> and <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_nav')?>">saisai_nav</a> functions
...
<p>The default installation of SAISAI CMS currently only comes with the English langauge.
However, localization efforts are underway on the <a href="https://github.com/daylightstudio/SAISAI-CMS-Languages" target="_blank">SAISAI CMS Languages GitHub repo</a>.
If you are interested in helping out with language support, just let us know or send us a pull request (we love those).</p>


<p>To add a language to SAISAI, you must <a href="https://github.com/daylightstudio/SAISAI-CMS-Languages" target="_blank">download the latest 1.0 language folder (if available).</a> and
place it's contents in the <span class="file">saisai/modules/saisai/language</span> folder. Then, you can set the <dfn>$config['language']</dfn> in the <span class="file">saisai/application/config.php</span>
...
<p>The following are some key areas in which you can utilize SAISAI's localization functionality.</p>
...
<p>The <a href="<?=user_guide_url('libraries/form_builder')?>">Form_builder class</a> is used throughout SAISAI CMS to create the forms used to manage module data. To allow for the localized labels in the module forms, the <var>lang_prefix</var> property can be used and will look for labels in language files using the format <var>form_label_{literal}{field_name}{/literal}</var>, if not label value is supplied.</p>

<h2>Data_table Class</h2>
<p>Similar to the Form_builder class, the <a href="<?=user_guide_url('libraries/data_table')?>">Data_table class</a> also has a <var>lang_prefix</var> property. This prefix is used for localizing the table column headers. The prefix is set in SAISAI to be the same as the Form_builder's which is <var>form_label_</var>.</p>
...
<p>The <a href="<?=user_guide_url('modules/simple')?>">js_localized</a> property can be added to your modules if you have have javascript that needs to use some localized text. 
	...
	If you are using a <a href="<?=user_guide_url('general/javascript#jqx')?>">jqx</a> type controller that extends the BaseSaisaiController.js,
