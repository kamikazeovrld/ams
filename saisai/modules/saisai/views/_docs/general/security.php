<h1>SAISAI Security</h1>
<p>SAISAI CMS has several security options you can change and they are listed below. In addition, cookie information is encrypted by default and you can specify your own encryption key in the configuration settings like any other
CI application.</p>

<p class="important">SAISAI CMS 1.0 improved the security hashing used for storing passwords in the database.</p>

<h2>SAISAI Configuration Security Settings</h2>
<p>The <a href="<?=user_guide_url('general/configuration')?>">SAISAI config</a> provides several security settings to:</p>
<ul>
	<li><strong>restrict_to_remote_ip</strong> - restrict SAISAI to only certain IP addresses (array only value so can include multiple).</li>
...
<ul>
	<li><strong>sanitize_input</strong> - cleans the input before inserting or updating the data source. 
		...
		The list of functions to sanitize the input is set by the <dfn>module_sanitize_funcs</dfn> <a href="<?=user_guide_url('general/configuration')?>">SAISAI configuration</a> value under the security settings.
