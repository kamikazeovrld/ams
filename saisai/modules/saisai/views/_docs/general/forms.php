<?=css('jquery.supercomboselect, markitup, jquery-ui-1.8.17.custom', 'saisai')?>
<script>
<?php $this->load->module_view(SAISAI_FOLDER, '_blocks/saisai_header_jqx'); ?>
</script>
<?=js('jqx/jqx', 'saisai')?>
<?=js('saisai/saisai.min', 'saisai')?>
<?php 
$CI->load->library('form_builder');
$CI->form_builder->load_custom_fields(APPPATH.'config/custom_fields.php');

//echo js('saisai/custom_fields.js', 'saisai');
echo js('jquery/plugins/jquery-ui-1.8.17.custom.min', 'saisai');
...
function form_builder_example($field, $params = NULL, $include_submit = FALSE)
{
 ...
 ?>
<h1>Forms</h1>
<p>One of the biggest features of SAISAI CMS is it's ability to create simple to complicated form interfaces for your models and layouts.
...
would need to be created on the frontend (e.g. jqx.config.saisaiPath, jqx.config.imgPath, ...etc), otherwise, you may see errors in the console like "jqx is not defined".
</p>
...
<p>SAISAI CMS 1.0 has added several new field types as well as made it easier to create custom form fields.
In previous versions, to create a custom field type, you needed to create a custom function or class method 
and use the '<a href="#custom">custom</a>' field type to render it. In SAISAI CMS 1.0, you can register those custom
...
<span class="file">saisai/application/config/custom_fields.php</span>. For example, we use this method to
associate the the datetime field type with the jQuery UI datepicker.

<p>Custom fields require a function or class method to render the field and an association to be made in the <span class="file">saisai/application/config/custom_fields.php</span> file (<a href="#association_parameters">this file is explained below</a>).
...
<p>By default, SAISAI CMS 1.0 provides several custom field types which are defined in the <span class="file">saisai/modules/saisai/libraries/Saisai_custom_fields.php</span> class.</p>
...
<div class="float_left">
	<h3>SAISAI Custom Field Types</h3>
	<ul>	
...
<div class="toggle_block_off">
	...
	The following additional parameters can be passed to this field type:</p>
	<ul>
		<li><strong>date_format</strong>: The PHP date format in which you want the date to appear. This can be set in the <span class="file">saisai/application/config/MY_config.php</span> as well as on the Form_builder object. The default is <dfn>'m/d/Y'</dfn></li>
...
<div class="toggle_block_off">
	...
	<ul>
		<li><strong>folder</strong>: determines which <span class="file">saisai/application/views/_blocks</span> subfolder to look in for displaying</li>
		...
		<li><strong>recursive</strong>: determines whether to recursively look for subfolders within the <span class="file">saisai/application/views/_blocks</span> folder</li>
...
<div class="toggle_block_off">
	...
	<ul>
		...
		<li><strong>accept</strong>: specifies which files are acceptable to upload. It will default to what is specified in your saisai configuration for "editable_asset_filetypes"</li>
	</ul>
...
<div class="toggle_block_off">
	<p>This field type is used for textareas and will use SAISAI's <dfn>$config['text_editor']</dfn> configuration to determine which editor to display in the field by default.
	The following additional parameters can be passed to this field type:</p>
	<ul>
...
<div class="toggle_block_off">
	<p>This field displays a dropdown of states to select from. It automatically pulls it's options from the <span class="file">saisai/application/config/states.php</span> config file.
	The following additional parameters can be passed to this field type:</p>
	<ul>
...
<div class="toggle_block_off">
	<p>This field type generates a dropdown select with the language values specified in MY_saisai.php.</p>
...
<p>Creating a custom field type requires an association be made in the <span class="file">saisai/application/config/custom_fields.php</span>
...
<h3>Example</h3>
<pre class="brush:php">
'class'		=> array(SAISAI_FOLDER => 'Saisai_custom_fields'), // key is the module, and the value is the class
...
'js'		=> array(
					SAISAI_FOLDER => // the module in which assets/js folder to look in
						'jquery/plugins/jquery.repeatable', // the path to the javascript file relative to the assets/js folder
						),
'js_function' => 'saisai.fields.template_field', // the name of the javascript function to execute upon rendering of the form
...
For example, SAISAI will automatically set a field with a name of 'pwd' or 'passwd' to be the 'password' type field.
...
<p>The first is to add them to the $config['representatives'] array in the <span class="file">saisai/application/config/custom_fields.php</span>:</p>
