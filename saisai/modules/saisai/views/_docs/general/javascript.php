<h1>Javascript</h1>
<p>SAISAI CMS provides several ways for you to insert javascript functionality for various aspects of the CMS. <a href="http://jquery.com" target="_blank">jQuery</a>
...
The following are topics which discuss adding javascript to SAISAI CMS:
</p>

<ul>
	<li><a href="#config">SAISAI Configuration</a></li>
...
<h2 id="config">SAISAI Configuration</h2>
<p>If you'd like to include a javascript file for every page in the CMS, you can add it like so in your <span class="file">saisai/application/config/MY_saisai.php</span> file:</p>
<pre class="brush:php">
...
$config['saisai_javascript'][] = array('my_js');
...
</pre>
...
<p>To add javascript for an entire module, you can add a 'js' parameter to your module's initialization parameters in your <span class="file">saisai/application/config/MY_saisai_modules.php</span> like so:</p>
<pre class="brush:php">
...
<p>SAISAI CMS 1.0 offers several new ways to incorporate javascript specific to your forms using the <a href="<?=user_guide_url('libraries/form_builder')?>">Form_builder</a> class:</p>
...
<h3>Custom Field</h3>
<p>One of the most powerful new features of SAISAI 1.0 is the addition of custom form fields with <a href="<?=user_guide_url('libraries/form_builder')?>">Form_builder</a>. A custom field is a combination of custom
has 2 main parts. The first being a method or function that renders the field type. SAISAI comes with several built in custom field types which are rendered using the <span class="file">saisai/modules/saisai/libraries/Saisai_custom_fields.php</span> file.
...
needed for the field. This configuration is done in the <span class="file">saisai/modules/saisai/config/custom_fields.php</span> file and can be done for your specific field types in the <span class="file">saisai/application/config/custom_fields.php</span> file.</p>
...
<p>SAISAI comes with several built in custom field types which can be viewed in the <span class="file">saisai/modules/saisai/libraries/Saisai_custom_fields.php</span> file and are configured with <a href="<?=user_guide_url('libraries/form_builder')?>">Form_builder</a> in the
<span class="file">saisai/modules/saisai/config/form_builder.php</span> file.</p>
...
$(function(){
	$("#state").change(function(e){
		$.get('.saisai_url('my_module/ajax/cities_by_state').', function(data){
...
<p><a href="<?=site_url('saisai/modules/'.SAISAI_FOLDER.'/assets/js/jqx/jqx.js')?>">jQX</a> is a small javascript MVC framework.
...
<p>There are additional module initialization parameters you can set in your <span class="file">saisai/application/confg/MY_saisai_modules.php</span>.if you are using a jQX controller which include:</p>
<ul>
	<li><strong>js_controller</strong>: the name of the file and thus controller to use. The default is the BaseSaisaiController.js</li>
	<li><strong>js_controller_path</strong>: the object/folder path to the controller. The default is <dfn>saisai.controller.BaseSaisaiController</dfn> (folder slashes are simply replaced with dots)</li>
	<li><strong>js_controller_params</strong>: an initialization object of parameters to pass to the controller.</li>
	<li><strong>js_localized</strong>: an array of localized string keys to be accessible via the <dfn>saisai.lang()</dfn> function</li>
</ul>
...
controller.MyModuleController = jqx.createController(saisai.controller.BaseSaisaiController, {
...
<p>The above example would assume that you have a jQX Controller in your advanced modules <span class="file">saisai/{module}/assets/js/controller</span> folder named MyModuleController.
If the folder you wanted to save it in was <span class="file">saisai/{module}/assets/js/myfolder</span>, you would need to change the objects name to <dfn>myfolder.MyModuleController</dfn>.
...
<ul>
	...
	<li><strong>jqx.config.jsPath:</strong> The path to the saisai modoules javascript folder which is <span class="file">/saisai/modules/saisai/assets/js/</span></li>
	<li><strong>jqx.config.imgPath:</strong> The path to the saisai modoules images folder which is <span class="file">/saisai/modules/saisai/assets/images/</span></li>
	...
	<li><strong>jqx.config.controllerName:</strong> The global name of the controller object which can be used by other scripts after the page is loaded. The default is "saisai"</li>
	<li><strong>jqx.config.jqxPath:</strong> The path to the jQX library which is <span class="file">/saisai/modules/saisai/assets/js/jqx</span></li>
	...
	<li><strong>jqx.config.saisaiPath:</strong> The path to the SAISAI CMS which by default is <span class="file">saisai/</span></li>
	<li><strong>jqx.config.modulePath:</strong> The path to the module which is <span class="file">saisai/{module}</span></li>
...
<p class="important">For more examples, look inside the saisai modules <kbd>js/saisai/controller</kbd> folder.</p>
