...
<p>SAISAI has added a way to deal with page redirects that doesn't require the need for .htaccess.
Redirects are configured in the <dfn>saisai/application/config/redirects.php</dfn> file.
...
<dfn>passive_redirects</dfn> will only work if there are no pages detected by SAISAI.
...
<p>Programmatically, you can access the instantiated <a href="<?=user_guide_url('libraries/saisai_redirects')?>">Saisai_redirects</a> object like so:</p>
<pre class="brush:php">
$this->saisai->redirects->execute();
</pre>


<p>You can also set global configuration values from within the <dfn>saisai/application/config/redirects.php</dfn> file like so:</p>
<pre class="brush:php">
...
<p>If you are needing to check the redirects, you can utilize <a href="<?=user_guide_url('libraries/saisai_redirects')?>">Saisai_redirects::test method</a> and create a simple view file at <span class="file">saisai/applications/views/redirects_test.php</span> with the following:</p>
<pre class="brush:php">
&lt;?php 
echo '<pre>';
print_r($CI->saisai->redirects->test());
echo '</pre>';
?&gt;
</pre>
...
Additionally, if you have the SAISAI configuration setting <dfn>$config['max_page_params']</dfn> with a value greater then 0, then you may run into issues where
