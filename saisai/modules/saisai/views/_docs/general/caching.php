...
excessive database queries and HTTP requests for javascript and CSS. To help with this, SAISAI provides several method to cache resources and thus speed up page speed.</p>

<p>Programmatically, you can access the SAISAI's cache methods through the instantiated <a href="<?=user_guide_url('libraries/saisai_cache')?>">Saisai_cache</a> object like so:</p>
<pre class="brush:php">
$this->saisai->cache->clear_all();
$this->saisai->cache->save('my_cache_id', $data);
</pre>

<h2>Page Caching</h2>
<p>SAISAI provides several options for caching your pages. The default setting, which can be altered in your <span class="file">saisai/application/config/MY_saisai.php</span>
...
<h2>Block Caching</h2>
<p>Similarly, <a href="<?=user_guide_url('libraries/saisai_blocks')?>">blocks</a> can also be cached as shown in the example below:</p>
<pre class="brush:php">
echo saisai_block(array('view' => 'my_block', 'cache' => TRUE));
</pre>
...
use limited PHP functionality. These template files get compiled and are stored in the <span class="file">saisai/application/cache/dwoo/compiled</span> folder.</p>
...
<pre class="brush:php">
php index.php saisai/manage/clear_cache
</pre>

<h2>Web Hooks</h2>
<p>The <a href="<?=user_guide_url('installation/configuration')?>">SAISAI configuration's <dfn>webhook_remote_ip</dfn> parameter</a> gives you the ability to set one or more IP addresses
that can be used to remotely call the saisai/manage/clear_cache. For example, say you use <a href="http://beanstalkapp.com" target="_blank">Beanstalk</a> to manage your GIT repositories and you would like to automatically clear the cache upon
...
<pre class="brush:php">
http://www.mysite.com/saisai/manage/clear_cache
</pre>