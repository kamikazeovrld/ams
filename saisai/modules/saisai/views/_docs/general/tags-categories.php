...
<p>SAISAI CMS 1.0 provides a generic tags and categories modules that can you incorporate into your own modules. <strong>They are hidden by default. </strong>
To turn them on, you must comment out the lines in the <span class="file">saisai/application/config/MY_saisai_modules.php</span> making them hidden.
...
This will use SAISAI's relationship table to store tag relationships and will create a multi-select for the module form in the CMS.
...
class Widgets_model extends Base_module_model {
	....
	public $has_many = array('tags' => 'saisai_tags_model');
	....
</pre>
...
<pre class="brush:php">
$tag = $this->saisai->tags->find_by_tag('my_tag');
...
class Widgets_model extends Base_module_model {
	....
	public $foreign_keys  = array('category_id' => array(SAISAI_FOLDER => 'saisai_categories_model', 'where' => array('context' => 'product_categories')));
	....
</pre>
...
<pre class="brush:php">
// also could use $this->saisai_categories_model->find_one(array('slug' => 'my_category')); assuming the model is loaded
$category = $this->saisai->categories->find_by_slug('my_category');
