...
<p>There are various ways to configure SAISAI CMS. Configuration files provide the most common way to make configurations. However,
new in SAISAI CMS 1.0 is the ability to create and manage your custom config settings in the CMS for <a href="<?=user_guide_url('modules/advanced')?>">Advanced Modules</a>.
</p>

<h2 id="difference">What's the Difference Between Configs and Settings?</h2>
<p>An Advanced Module (including the SAISAI Advanced Module) normally contains a single config file that is the same name as the Advanced Module
(e.g. <span class="file">saisai/modules/{module}/config/{module}.php</span>).
...
<pre class="brush:php">
...
echo $this->saisai->config('site_name');
</pre>

<p><strong>Config values are not editable in the CMS.</strong> To fix this issue, we've implemented <a href="<?=user_guide_url('libraries/saisai_settings')?>">Saisai_settings</a> which
...
<h2 id="saisai_specific">SAISAI Specific Application Config Files</h2>
<p>The following are configuration files specific to SAISAI CMS (and not editable in the CMS):</p>
<ul>
	<li><strong>asset:</strong> configuration parameters for the asset class</li>
	<li><strong>form_builder:</strong> used by SAISAI's <a href="<?=user_guide_url('libraries/form_builder')?>">Form_builder class</a> to set initialization preferences,
				in particular custom field types. By default includes configuration parameters from the <span class="file">saisai/modules/saisai/config/form_builder.php</span> file</li>
	<li><strong>google:</strong> used by SAISAI's <a href="<?=user_guide_url('helpers/google_helper')?>">Google helper</a></li>
	<li><strong>model:</strong> used by SAISAI's <a href="<?=user_guide_url('libraries/my_model')?>">MY_Model class</a></li>
	...
	<li><strong>MY_saisai_layouts:</strong> used to create the SAISAI <a href="<?=user_guide_url('general/layouts')?>">layouts for the CMS</a></li>
	<li><strong>MY_saisai_modules:</strong> used to create SAISAI <a href="<?=user_guide_url('modules/simple')?>">simple modules</a></li>
	<li><strong>MY_saisai:</strong> used to overwrite any of the default <a href="<?=user_guide_url('installation/configuration')?>">SAISAI configuration settings</a></li>
	...
	<li><strong>redirects:</strong> used to create <a href="<?=user_guide_url('general/redirects')?>">redirects</a> for SAISAI</li>
...
<ul>
	<li><strong>autoload:</strong> sets the packages, libraries, helpers, configs, languages and model files. 
		SAISAI automatically includes the <span class="file">saisai/modules/saisai</span> folder as a package path which allows
		it to store a lot of the files in that folder instead of the <span class="file">saisai/application</span> folder</li>
	...
	<li><strong>constants:</strong> includes both the Codeigniter and SAISAI specific constants and is loaded very early on in the bootstrap process</li>
	...
	<li><strong>hooks:</strong> the <a href="http://ellislab.com/codeigniter/user-guide/general/hooks.html" target="_blank">CodeIgniter Hooks</a> configuration. By default it includes the <span class="file">saisai/modules/saisai/config/saisai_hooks.php</span>
		file which contains SAISAI specific hooks. </li>
