...
<p>SAISAI CMS provides several ways for creating pages. To help illustrate, we'll pretend we are creating a simple website. Our website will have the following pages:</p>
<ul>
...
<p class="important">Make sure the configuration of <kbd>saisai_mode</kbd> is set to <dfn>AUTO</dfn> in the <dfn>application/config/MY_saisai.php</dfn> file.</p>
...
Often times you'll see <dfn>saisai_set_var('body', '');</dfn> in a layout. This is because the variable <dfn>$body</dfn> holds the contents of the view file (in this case the 'about' page).
</p>
...
<p>SAISAI still allows you to create pages using the standard MVC way in CodeIgniter. For the purposes of this tutorial, we will use a controller for the <dfn>about/contact</dfn> because it has a webform we may want to use for processing. We can also leverage the variables specified in the <dfn>application/views/_variables/</dfn> file like so:</p>
...
class About extends CI_Controller {
	...
	function contact()
	{
		...
		//... form code goes here
		$this->saisai->pages->render('about/contact', $vars);
	}
}
</pre>
...
<dfn>http://mysite.com/saisai</dfn> (where <strong>mysite.com</strong> is your domain) and entering the username of <dfn>admin</dfn> with the password (which is probably <dfn>admin</dfn> as well if you haven't changed it). Click on the <dfn>Pages</dfn> menu item on the left and follow the directions
<a href="<?=user_guide_url('modules/saisai/pages')?>">seen here</a> to create a page. For the <dfn>location</dfn> value type in <strong>about/team</strong> and for the <dfn>body</dfn> value, put in the value below, click save and then view the page:</p>
...
<ul>
	...
	<li>A model</li>
	<li>Modify the <dfn>application/config/MY_saisai_modules.php</dfn> file</li>
	<li>A view file</li>
...
<h3>MY_saisai_modules.php Configuration Change</h3>
<p>Add the following module to the <dfn>application/config/MY_saisai_modules.php</dfn> file: </p>
<pre class="brush: php">
...
The following view file uses the <a href="<?=user_guide_url('helpers/saisai_helper')?>">saisai_model</a> and
...
if (!empty($slug))
{
	$news_item = saisai_model('news', array('find' => 'one', 'where' => array('slug' => $slug)));
	if (empty($news_item)) show_404();
}
else
{
	$news = saisai_model('news');
}
?&gt;
...
&lt;?php saisai_set_var('page_title', $news_item->headline); ?&gt;
