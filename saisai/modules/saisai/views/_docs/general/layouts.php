<h1>Layouts</h1>
<p>Layouts in SAISAI are made up of static content and variable placeholders to be used by your pages. You can use layouts for both your <a href="#layouts_static_views">static view files</a> as well as pages you
...
<p>Programmatically, you can access information about the layouts through the instantiated <a href="<?=user_guide_url('libraries/saisai_layouts')?>">Saisai_layouts</a> object like so:</p>
<pre class="brush:php">
$this->saisai->layouts->options_list();
$this->saisai->layouts->get();
</pre>
...
<dfn>layout</dfn> in your variables file. The second is to assign a variable of <dfn>layout</dfn> using <a href="<?=user_guide_url('helpers/saisai_helper')?>">saisai_set_var</a>.
</p>
...
// using the saisai_set_var in your view file
&lt;?php saisai_set_var('layout', 'home')?&gt;
</pre>
...
<p class="important">The variable <dfn>$body</dfn> is is used to hold the contents of the static view file which gets merged into the layout. 
	Because of this, you'll often see <dfn>saisai_set_var('body', '')</dfn> in layout files.</p>

<h2 id="layouts_cms">Using Layouts in the CMS</h2>
<p>For a layout to appear in the SAISAI admin, a layout view file must exist in the <span class="file">saisai/application/views/_layouts/</span> folder AND
there must be an entry in the <span class="file">saisai/application/config/MY_saisai_layouts.php</span> file. The latter, assigns the variable fields to the layout and signals that the layout can be used to create pages in the admin.<p>

<p>As of version 1.0, there are two ways to assign layouts in the <span class="file">MY_saisai_layouts.php</span> file. The first being the old way of assigning the layout variables to the
...
<p>The second, and newer way, is to create a <a href="<?=user_guide_url('libraries/saisai_layouts#saisai_layout')?>">Saisai_layout</a> object and assign the layout variables to it. Additionally, you can assign the <dfn>label</dfn> (used for the dropdown in the admin),
...
$main_layout = new Saisai_layout('main');
...
Layout class by extending the <a href="<?=user_guide_url('libraries/saisai_layouts#saisai_layout')?>">Saisai_layout</a> class and modifying the <dfn>pre_process</dfn> and/or <dfn>post_process</dfn> method hooks. The <dfn>pre_process</dfn>
...
You can tell SAISAI where to look for the class by specifying <dfn>file</dfn>, <dfn>class</dfn>, <dfn>filepath</dfn> and <dfn>filename</dfn> parameters as displayed below:
</p>
...
<span class="file">saisai/application/config/MY_saisai_layouts.php</span><br />
<pre class="brush:php">
...
<span class="file">saisai/application/libraries/_layouts/Home_layout.php </span><br />
<pre class="brush:php">
...
<p>By default, SAISAI will assume the layout view file is the same name as the layout's key value (e.g. 'main'). To change it, you can assign the layout a different <dfn>file</dfn> value:</p>
<pre class="brush:php">
$config['layouts']['main'] = array(
...
<p>Block layouts are a way to associate layout form fields to particular blocks. This creates a whole new level of layout control for SAISAI layouts.
...
It assumes that you have a page layout of <span class="file">saisai/application/views/_layouts/test.php</span> and a block at <span class="file">saisai/application/views/_blocks/sections/image_right.php</span>:
</p>

<br />
<span class="file">saisai/application/config/MY_saisai_layouts.php</span></br >
<pre class="brush:php">
$test_layout = new Saisai_layout('test');
...
// Saisai layout block
$image_right = new Saisai_block_layout('image_right');
...
<br />
<span class="file">saisai/application/views/_layouts/test.php</span></br >
<pre class="brush:php">
		...
		&lt;div class=&quot;wrapper&quot;&gt;
			...
			&lt;article id=&quot;primary&quot;&gt;
				...
				&lt;?php foreach($sections as $section) : 
					...
					if (!empty($block_name)) :
				?&gt;
				&lt;?=saisai_block(&#039;sections/&#039;.$block_name, $section[&#039;block&#039;]) ?&gt;
				&lt;?php endif; ?&gt;
				&lt;?php endforeach; ?&gt;
			&lt;/article&gt;
		&lt;/div&gt;
...
<br />
<span class="file">saisai/application/views/_blocks/sections/image_right.php</span></br >
<pre class="brush:php">
&lt;section class=&quot;row&quot;&gt;
