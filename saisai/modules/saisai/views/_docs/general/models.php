...
$CI->inspection->initialize(array('file' => SAISAI_PATH.'core/MY_Model.php'));
$classes = $CI->inspection->classes();
$my_model = $classes['MY_Model'];
?>

<h1>Models</h1>
<p>There are two main classes SAISAI provides for creating models. The first, <a href="<?=user_guide_url('libraries/my_model')?>">MY_Model</a>,
...
<p class="important">Most modules in SAISAI extend the <a href="<?=user_guide_url('libraries/base_module_model')?>">Base_module_model</a> class,
...
<pre class="brush:php">
// loading from the saisai/application/libraries/custom_fields/My_asset_field.php
'image' => 'My_asset_field', 

// loading from the saisai/modules/my_module/libraries/custom_fields/My_text_field.php
'name' => array('my_module' => My_text_field'), 

// loading from the saisai/application/libraries/custom_fields/My_url_field.php and passing ina prefix_path of 'test'
...
<p class="important">By default, SAISAI will look in the "{module}/libraries/custom_fields" folder for the the classes to load.</p>
...
function if the property is a string value and if it is a date value will format the date by default to whatever is specified in the <span class="file">saisai/application/config/MY_config.php</span>.
</p>
<pre class="brush: php">
...
<p>SAISAI CMS 1.0 provides several ways to form model relationships by adding one or more of the following properties to your model:</p>
<ul>
...
say you have a products model. Each product can belong to only one category. In this case, we can use SAISAI's built in <a href="<?=user_guide_url('general/tags-categories#categories')?>">Categories</a>
module to make the relationship like so:</p>
<pre class="brush:php">
class Products_model extends Base_module_model
{
  public $foreign_keys  = array('category_id' => array(SAISAI_FOLDER => 'categories_model')));
...
<p class="important">The constant <dfn>SAISAI_FOLDER</dfn> is used as the array key above to point to the module in which to load the model (e.g. array(SAISAI_FOLDER => 'categories_model'))).
You can alternatively just use the name of the model as a string if the model exists in your application directory or you can use <dfn>'app'</dfn> instead of <dfn>SAISAI_FOLDER</dfn>
</p>
<br />
...
class Products_model extends Base_module_model
{
  public $foreign_keys  = array('category_id' => array(SAISAI_FOLDER => 'categories_model', 'where' => array('context' => 'products')));
...
$product = saisai_model('products', 'key', $id);
if (isset($product->id))
{
...
<p>The <dfn>has_many</dfn> model property allows you to assign many-to-many relationships between models without needing to setup a separate lookup table. SAISAI will automatically
save this relationship in the <dfn>saisai_relationships</dfn> table. If we continue with the products example above, a product may also need to be associated with multiple attributes,
or tags, like regions your product belongs to, or specific features. For this, you can use SAISAI's built in <a href="<?=user_guide_url('general/tags-categories#tags')?>">Tags</a> module to create relationships like so:</p>

<pre class="brush:php">
class Products_model extends Base_module_model
{
  public $has_many = array('attributes' => array(array(SAISAI_FOLDER => 'saisai_tags_model'));
...
class Products_model extends Base_module_model
{
  public $has_many = array('attributes' => array('model' => array(SAISAI_FOLDER => 'saisai_tags_model'));
...
class Products_model extends Base_module_model
{
  public $has_many = array('attributes' => array('model' => 'saisai_tags_model', 'module' => SAISAI_FOLDER)); //NOTE THE 'module' key
...
class Products_model extends Base_module_model
{
  public $has_many = array('attributes' => array('model' => array(SAISAI_FOLDER => 'saisai_tags_model', 'where' => 'category_id = 1'));
...
<p>The <dfn>belongs_to</dfn> model attribute is very similar to the <dfn>has_many</dfn>, but is done from the opposite perspective, which in this case would be from SAISAI's <a href="<?=user_guide_url('general/tags-categories#tags')?>">Tags</a> module.
SAISAI automatically creates the <dfn>belongs_to</dfn> relationship for the Tags module. This allows you to see which products belong to a specific tag in this example:</p>
<pre class="brush:php">
$slug = uri_segment(3); // assumption here that the 3rd segment has the slug
$tag = saisai_model('products', 'one', array('slug', $slug));
if (isset($tag->id))
{
	$tags_model = $product->get_attributes(TRUE); // NOTE THE DIFFERENCE HERE
...
<p class="important">
	By default, SAISAI uses the built-in relationships model to save relationship data. However, if you want to change the model, as well as the field names it uses to store the key information,
	you can pass the following	parameters in your <dfn>has_many</dfn> or <dfn>belongs_to</dfn> properties array: <dfn>saisai_relationships_model</dfn>, <dfn>foreign_key</dfn>, <dfn>candidate_key</dfn>. An Example is below:
</p>
<pre class="brush:php">
class Products_model extends Base_module_model
{
  public $has_many = array('attributes' => array('model' => array(SAISAI_FOLDER => 'saisai_tags_model'), 'saisai_relationships_model' => 'my_relationship_model', 'foreign_key' => 'my_foreign_key', 'candidate_key' => 'candidate_key'));
...
<p>SAISAI CMS provides a way for you to automatically serialize and unserialize data in a field. To do this, you can assign the field name to the <dfn>serialized_fields</dfn> array. This will automatically
