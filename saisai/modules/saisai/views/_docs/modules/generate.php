<h1>Generate</h1>
<p>SAISAI CMS provides a command line tool to help you auto generate starter files for your simple and advanced modules. To do so, navigate
...
<p>The default generated template files exist in the <span class="file">saisai/modules/saisai/views/_generated/{type}</span> folder where <dfn>{type}</dfn>
is either "advanced", "model" or "simple". You can overwrite these defaults by creating a <span class="file">saisai/application/views/_generated/{type}</span>
folder with files that correspond to the files specified in the <a href="<?=user_guide_url('installation/configuration')?>">SAISAI configuration file</a> under the "generated" parameter.
...
<pre class="brush:php">
php index.php saisai/generate/model examples

// generates multiple models at once
php index.php saisai/generate/model examples:examples2

// generates a model in an advanced module named examples
php index.php saisai/generate/model my_model examples
</pre>
...
<p>The following will create a model named "examples_model.php", generate a placeholder table (you'll need to modify), add the permisions to manage it in the CMS,  and will add it to the <span class="file">saisai/application/config/MY_saisai_modules.php</span>:</p>
<pre class="brush:php">
php index.php saisai/generate/simple examples
</pre>

<h2 id="advaned">Advanced Modules</h2>
<p>The following will create a directory named "test" in the <span class="file">saisai/modules/</span> folder, as well as create a permission and add it to the "modules_allowed" SAISAI configuration.
It will generate by default the files specified in the
<a href="<?=user_guide_url('installation/configuration')?>">SAISAI configuration file</a> under the <dfn>generated</dfn> parameter:</p>
<pre class="brush:php">
...
<pre class="brush:php">
php index.php saisai/generate/simple my_model examples
</pre>
...
<p>By default, the SAISAI generate command comes with several models that we thought were pretty common to many website projects and so we included them as samples&mdash;they are <dfn>news</dfn>, <dfn>events</dfn>, <dfn>careers</dfn>.
