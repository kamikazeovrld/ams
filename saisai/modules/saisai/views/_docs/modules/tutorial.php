...
<p>The following is a tutorial on creating <a href="<?=user_guide_url('modules/simple')?>"><strong>simple</strong></a> modules in SAISAI.
...
We will be using 3 data models (<dfn>articles_model</dfn>, <dfn>authors_model</dfn>, <dfn>saisai_tags_model</dfn>), and creating 2 modules (articles, authors).</p>
...
<ol>
  ...
  <li><a href="#categories_module">The SAISAI Tags Module</a></li>
...
<p>Before we get started with this tutorial, it is recommended that you <a href="<?=assets_path('saisai_modules_example.zip', 'docs', SAISAI_FOLDER)?>"><strong>download the final files of the example</strong></a> and look them over briefly so it is easier to follow along.</p>

<p>You will need to <a href="<?=user_guide_url('installation/db-setup')?>">setup an instance of SAISAI including the database</a> with a working admin login.
We will use that SAISAI database to store the additional tables required for our models in this tutorial.</p>

<p>We will be creating two database tables to support the 2 modules &mdash; an authors table and an articles table. The saisai_tags table is already created for us.</p>
...
<pre class="brush: php">
php index.php saisai/generate/simple authors
</pre>
...
<ul>
  ...
  <li><strong>add entry to MY_saisai_modules</strong>: will add an entry in the saisai/application/config/MY_saisai_modules.php file so it will appear in the CMS under the Modules area in the left menu.</li>
...
<p>The following is the SQL code for the table used by authors model. Copy this code and run this query to generate the table in the SAISAI database instance you are using for this tutorial.</p>
...
carry special meaning in SAISAI and will determine whether to display the content on the website.
</p>
...
<dfn>saisai/application/model</dfn> directory and name it <dfn>authors_model.php</dfn>. In that file we will be
...
require_once(SAISAI_PATH.'models/base_module_model.php');
...
<p>This is really all you need at the moment for the authors model so we will now focus on getting it it hooked up to the SAISAI admin.</p>

<h3>Hooking Up the Authors Module</h3>
<p>Now that our table and model have been created, we need to tell SAISAI about it.
To do that, open up the <dfn>application/config/MY_saisai_modules.php</dfn> file and add the following line.</p>
<pre class="brush: php">
...
<p>Now login to saisai (e.g. http://mysite.com/saisai), and you should see the module on the left under the MODULES section.</p>
...
<img src="<?=img_path('screens/authors_form.png', SAISAI_FOLDER)?>" class="screen" />
...
and run this query to generate the table in the SAISAI database instance you are using for this tutorial.</p>
...
create a new file in the <dfn>saisai/application/model</dfn> folder and call it <dfn>articles_model.php</dfn>.
...
<p>The next relationship we want to add to our articles model is a many-to-many relationship with SAISAI's built in <a href="<?=user_guide_url('general/tags-categories')?>">Tags module</a>.
  This means that an article can belong to many different tags and vice-versa.   To do this, we will take advantage of <a href="<?=user_guide_url('general/models#relationships')?>">SAISAI 1.0's relationships</a> and
...
class Articles_model extends Base_module_model {
    ...
    public $foreign_keys = array('author_id' => 'authors_model');
    public $has_many = array('tags' => array(SAISAI_FOLDER => 'saisai_tags_model'));
...
We will use the latter since this relationship is with a model in the <dfn>saisai</dfn> advanced module. By adding this <dfn>has_many</dfn> relationship, it will do a couple things.
...
<p>Similar to the authors module, we will now need to make an entry in the <dfn>application/config/MY_saisai_modules.php</dfn> file and add the following lines.</p>
<pre class="brush: php">
...
<ul>
  ...
  <li><strong>display_field</strong>: The <dfn>display_field</dfn> is the main field used to identify the record in the CMS. By default, SAISAI will choose the
    ...
    can specify an array of options that are available from SAISAI's <a href="<?=user_guide_url('installation/configuration')?>">module_sanitize_funcs</a> config parameter. In this case, we will use
    the <var>template</var> and <var>php</var> value that maps to SAISAI's <a href="<?=user_guide_url('helpers/my_string_helper#func_php_to_template_syntax')?>">php_to_template_syntax</a> and <a href="http://php.net/manual/en/function.htmlentities.php" target="_blank">htmlentities</a> function respectively.</li>
</ul>
...
<p class="important">The key column (e.g. <kbd>id</kbd>) is not seen in the SAISAI admin but is important to include because it is used to identify the row. </p>
...
<img src="<?=img_path('screens/articles_form.png', SAISAI_FOLDER)?>" class="screen" />


<h2 id="saisai_tags_module">The SAISAI Tags Module</h2>
<p>As we alluded to earlier, we will use SAISAI's built in <a href="<?=user_guide_url('general/tags-categories')?>">tags module</a> to create the many-to-many relationships model with the articles module.
The <dfn>saisai_tags_model</dfn>, along with it's cousin the <dfn>saisai_tags_categories</dfn> model are part of the SAISAI <a href="<?=user_guide_url('general/tags-categories')?>">tags and categories</a> modules that
...
<span class="file">saisai/application/config/MY_saisai_modules.php</span> and comment out or remove the following module overwrites:</p>
...
<p>Now that you've unhidden the categories module, go back to the admin and refresh. You should see a "Tags" module appear on the left menu. 
  Click on it and click the "Create" button. You will see that a SAISAI tag has the following fields:</p>
<img src="<?=img_path('screens/tags_create.png', SAISAI_FOLDER)?>" class="screen img_right" />
<ul>
  <li><strong>name</strong>: The name of the tag</li>
  <li><strong>category_id</strong>: The SAISAI category it belongs to. This can come in handy when you want to group tags together. For example, you could create an "articles" category to group the tags together.</li>
...
<img src="<?=img_path('screens/manage_permissions.png', SAISAI_FOLDER)?>" class="screen img_right" />
<p>Now that the modules are created, you will probably want to allow others to use them. To do this,
you will need to access to the permissions module to create permissions so users can be allowed to access those modules in SAISAI.
To do that</p>
<ol>
...
<h3>Unique Fields</h3>
<p>SAISAI model's can also declare fields that need to be unique in addition to the primary key. In this example, the article's <dfn>slug</dfn> value needs to be unique across all articles.</p>
<pre class="brush: php">
... 
class Articles_model extends Base_module_model {
...
<h3>Parsed FIelds</h3>
<p>By default, SAISAI will not parse the data for <a href="<?=user_guide_url('general/template-parsing')?>">templating syntax</a>.
...
<p>The tree method will render like the following in the SAISAI admin:</p>
<img src="<?=img_path('screens/articles_tree.png', SAISAI_FOLDER)?>" class="screen" />
...
<p>Now that our modules are created for SAISAI, we now can add the module's model data to our views.
...
&lt;?php else: ?&gt;
  ...
  &lt;h1&gt;Articles&lt;/h1&gt;
  &lt;?=saisai_edit(&#039;create&#039;, &#039;Create Article&#039;, &#039;articles&#039;)?&gt;
...
<a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_model')?>">saisai_model</a> to retrieve the article object. This line is the equivalent to:</p>
<pre class="brush: php">
...
<p>If no slug value is passed to the page, then a simple <a href="<?=user_guide_url('helpers/saisai_helper#func_saisai_model')?>">saisai_model</a> call will be used which will automatically grab all tags associated with articles.
...
This is because we still need to tell SAISAI to use this view file for any URI location of <strong>articles/{slug}</strong>.
There are several ways to do this <strong>and you only need to implement one of them below</strong>:
<ol>
  <li><strong>max_page_params</strong>: you can add the following to SAISAI configuration parameter to <span class="file">saisai/application/views/_variables/MY_saisai.php</span>
    <pre class="brush: php">
    $config['max_page_params'] = array('articles' => 1);
    </pre>
  </li>
  <li><strong>auto_search_views</strong>: you can add the following to SAISAI configuration parameter to <span class="file">saisai/application/views/_variables/MY_saisai.php</span>
    <pre class="brush: php">
    $config['auto_search_views'] = TRUE;
    </pre>

  </li>
  <li><strong>global variables file</strong>: you can add the following to <span class="file">saisai/application/views/_variables/global.php</span>
    <pre class="brush: php">
    $pages['articles/:any'] = array('view' => 'articles');
    </pre>
  </li>
  <li><strong>articles variables file</strong>: since this view file exists at the first URI segment of "articles" you can create a separate <span class="file">saisai/application/views/_variables/articles.php</span> variables file add the following:
    <pre class="brush: php">
    $vars['view'] = 'articles';
    </pre>
  </li>
...
<p>The very last thing we will do is add <a href="<?=user_guide_url('general/inline-editing')?>">inline editing</a> to our view. To do this we use the <a href="helpers/saisai_helper">saisai_edit</a> function. We'll
place that function right inside the &lt;h1&gt; tag in the article view and inside the &lt;li&gt; in the list view. </p>

<pre class="brush: php">
...
&lt;h2&gt;&lt;?=saisai_edit($article)?&gt;&lt;?=$article-&gt;title?&gt;&lt;/h2&gt;
...
</pre>

<p>Additionally, you may add items in the context of a page. To do that, you pass <dfn>create</dfn> as the first parameter.</p>
<pre class="brush: php">
&lt;h2&gt;&lt;?=saisai_edit('create', 'Create', 'articles')?&gt;&lt;?=$article-&gt;title?&gt;&lt;/h2&gt;
</pre>
<p>Next, make sure the SAISAI bar located in the upper right of the page has the pencil icon toggled on. You should now see pencil icons that when clicked, will allow you to edit or add article data from within the context of the page.</p>
<img src="<?=img_path('screens/page_inline_editing.jpg', SAISAI_FOLDER)?>" class="screen" />
