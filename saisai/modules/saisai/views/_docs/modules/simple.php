...
<p>Modules in SAISAI can range from simple forms to a complex, interactive, multi-page interface.
...
After your model has been created, you can activate and configure the module in the <dfn>config/MY_saisai_modules.php</dfn> file like so:</p>
...
<table border="0" cellspacing="1" cellpadding="0" class="tableborder">
	<tbody>
		...
		<tr>
			...
			<td>None</td>
			<td>the URI path to the module that will appear after <dfn>saisai</dfn>.</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>The default is BaseSaisaiController</td>
			<td>None</td>
		...
		<tr>
			...
			<td>The default is the path to the saisai module's asset folder (e.g. saisai/modules/saisai/assets/js/</td>
			<td>None</td>
		...
		<tr>
			...
			<td>The form level actions to include in the toolbar. 
				The <dfn>others</dfn> action is a key / value array with the key being the SAISAI URI and the value being the button label.
		...
		<tr>
			...
			<td>TRUE</td>
			<td>Boolean TRUE/FALSE, string of "xss", "php", "template" OR "entities", OR an array for multiple values e.g. array('xss', 'php', 'template', 'entities'). See the SAISAI <a href="<?=user_guide_url('general/configuration')?>">configuration</a> for changing the different sanitization callbacks.</td>
...
class which is a child of the MY_Model class and is found in the <dfn>modules/saisai/model</dfn> folder.
...
<p>Module overwrites allow for you to overwrite existing module parameters. For example, if you'd like to overwrite the built-in SAISAI saisai_pages_model to incorporate your own model hook you've added to the MY_pages_model,
