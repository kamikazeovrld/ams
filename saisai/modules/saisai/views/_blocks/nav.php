	<div id="saisai_left_panel">
		<div id="saisai_left_panel_inner">
<?php 
	// // Get all modules
	$modules = $this->saisai->modules->get();
	$mods = $icons = array();
	...
	foreach($nav as $section => $nav_items)
	{
		if (is_array($nav_items))
		{
			...
			foreach($nav_items as $key => $val)
			{
				...
				if (($this->saisai->auth->has_permission($perm)) || $perm == 'dashboard')
				{
					if  (!$header_written)
					{
					...
					echo "><a href=\"".saisai_url($url)."\" class=\"ico ".$icon."\">".$val."</a></li>\n";
					$header_written = TRUE;
				} 
			}
		}
