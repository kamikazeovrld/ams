	...
	<table border="0" cellspacing="0" cellpadding="0">
		<tbody>
			<tr>
				...
				<?php endif; ?>
				<td><a href="<?=saisai_url($this->module_uri.'/reset_page_state', FALSE)?>" class="reset"></a></td>
				<td>
					<div class="search_input<?php if ( ! empty($this->advanced_search)) : ?> advanced<?php endif; ?>">
						...
						<?php if ( ! empty($this->advanced_search)) : ?>
						<a href="#" id="adv-search-btn" title="<?=lang('adv_search')?>"><img src="<?=saisai_url('modules/saisai/assets/images/th_arrow_desc.png')?>" /></a>
						<div class="adv_search">
							<p><strong><?=lang('adv_search')?></strong></p>
							<?php if (!empty($more_filters)) : ?>
							<?=$more_filters?>
							<?php endif; ?>
							<p><?=$this->form->submit(lang('btn_search'), 'search')?> &nbsp;&nbsp; <a href="#" id="adv-search-close"><?=lang('viewpage_close')?></a> &nbsp;&nbsp; <a href="<?=saisai_url($this->module_uri.'/reset_page_state', FALSE)?>"><?=lang('reset_search')?></a></p>
						</div>
						<?php endif; ?>
					</div>
				</td>
...
<div class="buttonbar" id="action_btns">
	...
	<ul>
		...
		<?php if (!empty($this->list_actions)) : ?>
			...
			?>
			<?php if ($this->saisai->auth->has_permission($this->permission, $action)) : ?>
			<li><?=anchor(saisai_url($action), $label, array('class' => 'ico ico_'.$lang_key))?></li>
			<?php endif; ?>
			<?php endforeach; ?>
		<?php endif; ?>
		...
		<?php if ($this->saisai->auth->module_has_action('delete') && $this->saisai->auth->has_permission($this->permission, 'delete')) : ?>
		<li><a href="#" class="ico ico_delete" id="multi_delete"><?=lang('btn_delete_multiple')?></a></li>
		<?php endif; ?>
		
		<?php if ($this->exportable AND $this->saisai->auth->has_permission($this->permission, 'export')) : ?>
			<li><a href="<?=saisai_url($this->module_uri.'/export')?>" class="ico ico_export" id="export_data"><?=lang('btn_export_data')?></a></li>
		<?php endif; ?>
		
		<?php if ($this->saisai->auth->module_has_action('create') AND $this->saisai->auth->has_permission($this->permission, 'create')) : ?>
		<li class="end"><a href="<?=saisai_url($create_url)?>" class="ico ico_create"><?=$this->create_action_name?></a></li>
		<?php endif; ?>
	</ul>
