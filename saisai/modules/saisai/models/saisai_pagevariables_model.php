 ...
 *
 * <strong>Saisai_pagevariables_model</strong> is used for storing page variable data
 * 
 * @package		SAISAI CMS
 * @subpackage	Models
 * @category	Models
 * @author		David McReynolds @ Saisai
 * @link		http://docs.saisai.co/models/saisai_pagevariables_model
 */

require_once('base_module_model.php');

class Saisai_pagevariables_model extends Base_module_model {
	...
	public $foreign_keys = array('page_id' => array(SAISAI_FOLDER => 'saisai_pages_model')); // Add foreign key relationship to saisai_pages_model
	...
	{
		parent::__construct('saisai_pagevars');
	}
	...
	{
		$this->db->select($this->_tables['saisai_pagevars'].'.*, '.$this->_tables['saisai_pages'].'.layout, '.$this->_tables['saisai_pages'].'.location, '.$this->_tables['saisai_pages'].'.published AS page_published');
		$this->db->join($this->_tables['saisai_pages'], $this->_tables['saisai_pages'].'.id = '.$this->_tables['saisai_pagevars'].'.page_id', 'left');
	...
	{
		$where = array($this->_tables['saisai_pages'].'.location' => $location, 'name' => $name);
		if (!empty($lang))
		{
			$where['language'] = $lang;
		}
	...
	{
		$where = array($this->_tables['saisai_pages'].'.location' => $location);
		if (!empty($lang))
		{
			$where['language'] = $lang;
		}
		...
		if ($include_pagevars_object)
		{
			...
			if (!empty($objs))
			{
				$data['pagevar'] = new Saisai_pagevar_helper();
	...
	public function form_fields($values = array(), $related = array())
	{
		...
		{
			$page = $CI->saisai->pages->find($values['page_id']);
			if (isset($page->id))
			{
				$layout = $this->saisai->layouts->get($page->layout);
	...
	{
		$CI =& get_instance();
		$lang_options = $CI->saisai->config('languages');
		
		$this->db->select($this->_tables['saisai_pagevars'].'.*, '.$this->_tables['saisai_pages'].'.layout, '.$this->_tables['saisai_pages'].'.location, '.$this->_tables['saisai_pages'].'.published AS page_published');
		$this->db->join($this->_tables['saisai_pages'], $this->_tables['saisai_pages'].'.id = '.$this->_tables['saisai_pagevars'].'.page_id', 'left');
		$this->db->where(array($this->_tables['saisai_pagevars'].'.active' => 'yes'));
		if ($this->honor_page_status AND !defined('SAISAI_ADMIN'))
		{
			$this->db->where(array($this->_tables['saisai_pages'].'.published' => 'yes'));
		}
	}
}
...
/**
 * SAISAI page variable record object
 *
 * @package		SAISAI CMS
...
/**
 * The Saisai_pagevar_helper object.
 * 
