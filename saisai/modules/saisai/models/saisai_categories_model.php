 ...
 *
 * <strong>Saisai_categories_model</strong> is used for managing SAISAI categories in the CMS
 * 
 * @package		SAISAI CMS
 * @subpackage	Models
 * @category	Models
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/models/saisai_categories_model
 */

require_once('base_module_model.php');

class Saisai_categories_model extends Base_module_model {
	...
	{
		parent::__construct('saisai_categories'); // table name
	}
	...
	public function list_items($limit = NULL, $offset = NULL, $col = 'nav_key', $order = 'desc', $just_count = FALSE)
	{
		...
		$this->db->join($table.' AS p', $this->tables('saisai_categories').'.parent_id = p.id', 'left');
	...
	public function tree($just_published = FALSE)
	{
		...
		{ 
			...
			$return[] = array('id' => $category['id'], 'label' => $category['name'], 'parent_id' => $category['parent_id'], 'location' => saisai_url('categories/edit/'.$category['id']), 'attributes' => $attributes);
		}
		return $return;
	}
	...
	{	
		...
		$fields['parent_id'] = array('type' => 'select', 'model' => 'saisai_categories', 'first_option' => lang('label_select_one'));
...
class Saisai_category_model extends Base_module_record {
	...
	public function initialize(&$parent, $fields = array())
	{
		...
		$modules = $this->_CI->saisai->modules->get(NULL, FALSE);
		...
		foreach($modules as $module)
		{
			...
			if (!empty($model->foreign_keys))
			{
				...
				foreach($model->foreign_keys as $key => $mod)
				{
					...
					if (is_array($mod) AND isset($mod[SAISAI_FOLDER]) AND ($mod[SAISAI_FOLDER] == 'saisai_categories_model' OR $mod[SAISAI_FOLDER] == 'categories'))
					{
