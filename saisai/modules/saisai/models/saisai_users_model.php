 ...
 *
 * <strong>Saisai_users_model</strong> is used for managing SAISAI users in the CMS
 * 
 * @package		SAISAI CMS
 * @subpackage	Models
 * @category	Models
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/models/saisai_users_model
 */

require_once('base_module_model.php');

class Saisai_users_model extends Base_module_model {
	...
	public $has_many = array('permissions' => array('model' => array(SAISAI_FOLDER => 'saisai_permissions_model'))); // Users have a "has_many" relationship with permissions
	...
	{
		parent::__construct('saisai_users');
	}
	...
	/**
	 * Determines whether the passed user name and password are valid for SAISAI 0.93
	 *
	 * @access	public
	 * @param	string	The user name
	 * @param	string	The password
	...
	public function list_items($limit = NULL, $offset = NULL, $col = 'email', $order = 'desc', $just_count = FALSE)
	{
		...
		if (!$CI->saisai->auth->is_super_admin())
		{
	...
	public function form_fields($values = array(), $related = array())
	{
		...
		$lang_dirs = list_directories(SAISAI_PATH.'language/', array(), FALSE);
		...
		if ($CI->saisai->auth->has_permission('permissions') AND
			...
			($CI->saisai->auth->has_permission('permissions') AND empty($values['id']))) // for creating new users so as to show permissions if you are a super admin
			)
		{
	...
	public function _create_permission_fields($params = array())
	{
		...
		$perms_list = array();
		if ($CI->saisai->auth->is_super_admin() OR $CI->saisai->auth->has_permission('permissions'))
		{
			$perms_list = $CI->saisai_permissions_model->find_all_array_assoc('name', array('active' => 'yes'), 'name asc');
		}
		else
		{
			$auth_user_id = $CI->saisai->auth->user_data('id');
		...
		{
			$user_perms = $CI->saisai_permissions_model->find_within($_POST['permissions'], array(), NULL, NULL, NULL, 'name');
		}
	...
	{
		$CI =& get_instance();
		$valid_user = $CI->saisai->auth->valid_user();
	...
	public function on_after_save($values)
	{
		...
		if (isset($values['id'], $user['id']) AND $values['id'] == $user['id'])
		{
			...
			{
				$CI->saisai->auth->set_user_data('password', $values['password']);
			}

			if (!empty($values['language']))
			{
				$CI->saisai->auth->set_user_data('language', $values['language']);
			}
			
		}

		if ( ! empty($this->normalized_save_data['new_password'])) {
			$this->saisai->logs->write(lang('auth_log_cms_pass_reset', $values['user_name'], $this->input->ip_address()), 'debug');
		}
	...
	protected function _send_email($id)
	{
		...
		if (!empty($id) AND !has_errors() AND isset($_POST['send_email']) AND (!empty($_POST['password']) OR !empty($_POST['new_password'])))
		{
			...
			$msg = lang('new_user_email', site_url('saisai/login'), $CI->input->post('user_name'), $password);
			$params['to'] = $CI->input->post('email');
			$params['subject'] = lang('new_user_email_subject');
			$params['message'] = $msg;
			$params['use_dev_mode'] = FALSE;
			if (!$CI->saisai->notification->send($params))
			{
				$CI->saisai->logs->write($CI->saisai->notification->last_error(), 'debug');
...
class Saisai_user_model extends Base_module_record {
