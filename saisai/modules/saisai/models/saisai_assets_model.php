 ...
 * Extends CI_Model
 *
 * <strong>Saisai_assets_model</strong> is used for managing asset data with the file system which includes retrieving and deleting images, pdfs, etc.
 * 
 * @package		SAISAI CMS
 * @subpackage	Models
 * @category	Models
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/models/saisai_assets_model
 */

// not pulling from the database so just extend the normal model
require_once(SAISAI_PATH.'libraries/Validator.php');

class Saisai_assets_model extends CI_Model {
	...
	public function list_items($limit = null, $offset = 0, $col = 'name', $order = 'asc')
	{
		...
		$asset_dir = $CI->saisai->assets->dir($group_id);
		
		$assets_path = $CI->asset->assets_server_path().$asset_dir.DIRECTORY_SEPARATOR;
		
		$exclude = $CI->saisai->config('assets_excluded_dirs');
		$exclude[] = 'index.html';
	...
	protected function _delete($file)
	{
		...
		while(!$end)
		{
			...
			$excluded_asset_folders = $CI->saisai->assets->excluded_asset_server_folders();
	...
	public function form_fields($values = array())
	{
		$CI =& get_instance();
		$fields = array();
		$editable_asset_types = $CI->saisai->config('editable_asset_filetypes');
		...
		$fields['asset_folder'] = array('label' => lang('form_label_asset_folder'), 'type' => 'select', 'options' => $CI->saisai->assets->dirs(), 'comment' => lang('assets_comment_asset_folder'));
		$fields['userfile_file_name'] = array('label' => lang('form_label_new_file_name'), 'comment' => lang('assets_comment_filename'));
		if ($CI->config->item('assets_allow_subfolder_creation', 'saisai'))
		{
