 ...
 *
 * <strong>Saisai_permissions_model</strong> is used for managing SAISAI users in the CMS
 * 
 * @package		SAISAI CMS
 * @subpackage	Models
 * @category	Models
 * @author		David McReynolds @ Saisai
 * @link		http://docs.saisai.co/models/saisai_permissions_model
 */

require_once('base_module_model.php');

class Saisai_permissions_model extends Base_module_model {
	...
	public $belongs_to = array('users' => array('model' => array(SAISAI_FOLDER => 'saisai_users_model'), 'where' => array('super_admin' => 'no')));	// Permissions have a "belong_to" relationship with users
	...
	{
		parent::__construct('saisai_permissions');
	}
	...
	public function tree()
	{
		...
		$perms_list = $CI->saisai_permissions_model->find_all_array_assoc('name', array(), 'name asc');
		$perms = array();
		...
		foreach($perms_list as $perm => $perm_val)
		{
			...
			$perms[$perm] = array('label' => $perm_val['description'], 'parent_id' => $parent_id, 'location' => saisai_url('permissions/edit/'.$perm_val['id']), 'attributes' => $sub_attributes);
		}
		return $perms;
	}
	// --------------------------------------------------------------------
	...
	public function on_after_post($values)
	{
		...
		if (isset($data['other_perms']) AND is_array($data['other_perms']) AND !empty($values['name']))
		{
			$module = $values['name'];
			$CI =& get_instance();
			$other_perms = $CI->saisai->permissions->create_simple_module_permissions($module, $data['other_perms']);
			...
			if (!empty($users))
			{
				...
				$CI->load->module_model(SAISAI_FOLDER, 'saisai_users_model');
				foreach($users as $user_id)
				{
					$user = $CI->saisai_users_model->find_by_key($user_id);
					if (isset($user->id))
					{
...
class Saisai_permission_model extends Data_record {
}