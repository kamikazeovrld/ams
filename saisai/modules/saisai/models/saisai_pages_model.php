 ...
 *
 * <strong>Saisai_pages_model</strong> is used for managing SAISAI users in the CMS
 * 
 * @package		SAISAI CMS
 * @subpackage	Models
 * @category	Models
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/models/saisai_pages_model
 */

require_once('base_module_model.php');

class Saisai_pages_model extends Base_module_model {
	...
	{
		parent::__construct('saisai_pages');
	}
	...
	public function related_items($values = array())
	{
		...
		if ($CI->saisai->modules->get('navigation')->info('disabled') === TRUE) return '';

		$CI->load->module_model(SAISAI_FOLDER, 'saisai_navigation_model');
		

		$where['location'] = $values['location'];
		$related_items = $CI->saisai_navigation_model->find_all_array_assoc('id', $where);
		$return = array();
		...
		else if (!empty($values['location']) AND $this->saisai->auth->has_permission('navigation', 'create'))
		{

			$return['navigation'] = array();
			$label = (!empty($values['page_vars']['page_title'])) ? $values['page_vars']['page_title'] : '';
			$parent_id = 0;
			$group_id = $CI->saisai->config('auto_page_navigation_group_id');
		...
		$view = $this->load->module_view(SAISAI_FOLDER, '_blocks/related_items_array', array('related_items' => $return), TRUE);
		$layout = $CI->saisai->layouts->get($values['layout']);
	...
	public function tree($just_published = FALSE)
	{
		...
		foreach($pages as $key => $val)
		{
			...
			$return[$key]['location'] = saisai_url('pages/edit/'.$val['id']);
		}
	...
	public function form_fields($values = array(), $related = array())
	{
		...
		$fields['layout']['options'] = $CI->saisai->layouts->options_list();
		...
		// set language field
		if ($CI->saisai->language->has_multiple())
		{
			$fields['language'] = array('type' => 'select', 'options' => $this->saisai->language->options(), 'order' => 4);
		}
		else
		{
			$fields['language'] = array('type' => 'hidden', 'value' => $this->saisai->language->default_option());
		}
		...
		// when the saisai mode is views, use a drop-down of views instead of manually entering locations
		if ($CI->saisai->config('saisai_mode') == 'views')
		{
			$views = $CI->saisai->pages->views();
	...
	{
		$this->delete_related(array(SAISAI_FOLDER => 'saisai_pagevariables_model'), 'page_id', $where);
	}
	...
	{
		$CI =& get_instance();
		$CI->load->module_model(SAISAI_FOLDER, 'saisai_pagevariables_model');
		...
		$CI->saisai_pagevariables_model->delete(array('page_id' => $ref_id));
		$page_variables_saved = $CI->saisai_pagevariables_model->save($archive['variables']);
	...
	public function replace($replace_id, $id, $delete = TRUE)
	{
		...
		$CI->saisai_pagevariables_model->db()->trans_begin();

		// retrieve new variables
		$new_values = $CI->saisai_pagevariables_model->find_all_array(array('page_id' => $id));

		// delete old variables
		$CI->saisai_pagevariables_model->delete(array('page_id' => $replace_id));
		$saved = TRUE;
		foreach($new_values as $var)
		{
			$var['page_id'] = $replace_id;
			if (!$CI->saisai_pagevariables_model->save($var))
			{
				$saved = FALSE;
			}
		}
		...
		if ($CI->saisai_pagevariables_model->db()->trans_status() === FALSE)
		{
			$saved = FALSE;
		    $CI->saisai_pagevariables_model->db()->trans_rollback();
		}
		else
		{
		    $CI->saisai_pagevariables_model->db()->trans_commit();
		}
	...
	{
		$this->db->join($this->_tables['saisai_users'], $this->_tables['saisai_users'].'.id = '.$this->_tables['saisai_pages'].'.last_modified_by', 'left');
		$this->db->select($this->_tables['saisai_pages'].'.*, '.$this->_tables['saisai_users'].'.user_name, '.$this->_tables['saisai_users'].'.first_name, '.$this->_tables['saisai_users'].'.last_name, '.$this->_tables['saisai_users'].'.email, CONCAT('.$this->_tables['saisai_users'].'.first_name, '.$this->_tables['saisai_users'].'.last_name) AS full_name', FALSE);
	}
}

class Saisai_page_model extends Base_module_record {
	...
	function get_variables($language = NULL)
	{
		...
		return $this->lazy_load(array('page_id' => $this->id), array(SAISAI_FOLDER => 'saisai_pages_model'), TRUE, $params);
	}
}
