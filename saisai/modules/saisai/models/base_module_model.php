...
/**
 * Extends MY_Model to be specific to SAISAI modules
 *
 ...
 * @category	Models
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/models/base_module_model
 */
...
class Base_module_model extends MY_Model {
	...
	public function __construct($table = NULL, $params = NULL)
	{
		...
		$CI->load->module_language(SAISAI_FOLDER, 'saisai');
		...
		if (!isset($module)) $module = SAISAI_FOLDER;
		
		$saisai_tables = array();
		$module_tables = array();
		$config_tables = array();
		
		// first load the SAISAI config so that we can get the tables
		$CI->config->module_load(SAISAI_FOLDER, 'saisai', TRUE);
		$saisai_tables = $CI->config->item('tables', 'saisai');
		...
		self::$tables = array_merge(self::$tables, $config_tables, $module_tables, $saisai_tables);
	...
	protected function _list_items_query()
	{
		...
		foreach($this->filters as $key => $val)
		{
			...
			if (!empty($val)) 
			{
				...
				// from imknight https://github.com/daylightstudio/SAISAI-CMS/pull/113#commits-pushed-57c156f
	...
	protected function _tree($prop = NULL)
	{
		...
		if (!empty($this->foreign_keys) OR !empty($this->has_many) OR !empty($this->belongs_to))
		{
			...
			$rel_module_obj =  $CI->saisai->modules->get($rel_module, FALSE);
			...
			$module_obj =  $CI->saisai->modules->get($module, FALSE);
			if (!$module_obj)
			{
				return array();
			}
			...
			{
				$CI->load->module_model(SAISAI_FOLDER, 'saisai_relationships_model');
				$groups = $rel_model->find_all_array(array(), $rel_col.' '.$rel_order);
				$children = $CI->saisai_relationships_model->find_by_candidate($this->table_name(), $rel_model->table_name(), NULL, 'array');
			...
			else if ($prop == 'belongs_to')
			{
				$CI->load->module_model(SAISAI_FOLDER, 'saisai_relationships_model');
				$groups = $rel_model->find_all_array(array(), $rel_col.' '.$rel_order);
				$children = $CI->saisai_relationships_model->find_by_candidate($rel_model->table_name(), $this->table_name(), NULL, 'array');
			...
			foreach($children as $child)
			{
				...
				$return['g'.$child[$g_key_field].'_c_'.$child[$key_field]] = array('parent_id' => $child[$key_field], 'label' => $child[$display_field], 'location' => saisai_url($module_obj->info('module_uri').'/edit/'.$child[$loc_field]), 'attributes' => $attributes);
			}
			...
			foreach($groups as $group)
			{
				if (isset($used_groups[$group[$rel_key_field]]))
				{
					...
					$return[$group[$rel_key_field]] = array('id' => $group[$rel_key_field], 'parent_id' => 0, 'label' => $group[$rel_display_field], 'location' => saisai_url($rel_module_obj->info('module_uri').'/edit/'.$group[$rel_key_field]), 'attributes' => $attributes);
				}
	...
	{
		$CI =& get_instance();
		$CI->load->module_model(SAISAI_FOLDER, 'saisai_archives_model');
		...
		if ($saved = $this->saisai_archives_model->save($save))
		{
			$num_versions = $this->saisai_archives_model->record_count(array('table_name' => $this->table_name, 'ref_id' => $ref_id));
			if ($num_versions > $CI->config->item('max_number_archived', 'saisai') )
			{
				$delete_version = ($last_archive_version - $CI->config->item('max_number_archived', 'saisai')) + 1;
				$where = array('table_name' => $this->table_name, 'ref_id' => $ref_id, 'version' => $delete_version);
				$this->saisai_archives_model->delete($where);
			}
		}
		return $saved;
	}
	...
	public function get_last_archive($ref_id, $all_data = FALSE)
	{
		$CI =& get_instance();
		$CI->load->module_model(SAISAI_FOLDER, 'saisai_archives_model');
		$archive = $this->saisai_archives_model->find_one_array(array('table_name' => $this->table_name, 'ref_id' => $ref_id), 'version_timestamp desc');
	...
	public function get_archive($ref_id, $version = NULL, $all_data = FALSE)
	{
		...
		$archive = $CI->saisai_archives_model->find_one_array($where);
		$return = $archive;
		$return['data'] = array();
		if (!empty($archive))
		{
	...
	/**
	 * Add SAISAI specific changes to the form_fields method
	 *
	 * @access	public
	...
	public function _common_query($display_unpublished_if_logged_in = NULL)
	{
		...
		if ((!defined('SAISAI_ADMIN') AND $display_unpublished_if_logged_in === FALSE) OR ($display_unpublished_if_logged_in AND !is_saisaiified()))
		{
