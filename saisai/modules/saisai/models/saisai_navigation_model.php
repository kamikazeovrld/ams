 ...
 *
 * <strong>Saisai_navigation_model</strong> is used for managing SAISAI users in the CMS
 * 
 * @package		SAISAI CMS
 * @subpackage	Models
 * @category	Models
 * @author		David McReynolds @ Daylight Studio
 * @link		http://docs.getsaisaicms.com/models/saisai_navigation_model
 */

require_once('base_module_model.php');

class Saisai_navigation_model extends Base_module_model {
	...
	public $record_class = 'Saisai_navigation_item'; // The name of the record class
	...
	{
		parent::__construct('saisai_navigation');
	...
	{
		$where[$this->_tables['saisai_navigation'].'.location'] = $location;
	...
	{
		$where[$this->_tables['saisai_navigation'].'.nav_key'] = $nav_key;
		return $this->_find_by_array($where, $group_id, $lang);
	}
	...
	protected function _find_by_array($where, $group_id = 1, $lang = NULL)
	{
		...
		if (!empty($group_id))
		{
			...
			{
				$where[$this->_tables['saisai_navigation_groups'].'.name'] = $group_id;
			}
			else
			{
	...
	public function tree($just_published = FALSE)
	{
		...
		foreach($data as $key => $val)
		{
			...
			$return[$key]['location'] = saisai_url('navigation/edit/'.$val['id']);
		}
	...
	{
		$where = (is_string($group_id)) ? array($this->_tables['saisai_navigation_groups'].'.name' => $group_id) : array($this->_tables['saisai_navigation'].'.group_id' => $group_id);
		if (!empty($lang))
		{
			$where['language'] = $lang;
		}
	...
	public function max_id()
	{
		...
		$query = $this->db->get($this->_tables['saisai_navigation']);
	...
	public function form_fields($values = array(), $related = array())
	{
		...
		$CI =& get_instance();
		// navigation group
		if (empty($CI->saisai_navigation_groups_model)){
			$CI->load->module_model(SAISAI_FOLDER, 'saisai_navigation_groups_model');
		}
		$CI->load->helper('array');
		
		$group_options = $CI->saisai_navigation_groups_model->options_list();
	...
	//		'class' => 'add_edit navigation_group', 
		...
		if (empty($CI->saisai_pages_model))
		{
			$CI->load->module_model(SAISAI_FOLDER, 'saisai_pages_model');
		}
		...
		if ($CI->saisai->language->has_multiple())
		{
			$fields['language'] = array('type' => 'select', 'options' => $CI->saisai->language->options());
		}
		else
		{
			$fields['language'] = array('type' => 'hidden', 'value' => $CI->saisai->language->default_option());
		}
	...
	{
		$data = $this->find_one_array(array($this->_tables['saisai_navigation'].'.id' => $parent_id));
		if (!empty($data))
		{
	...
	public function on_before_clean($values)
	{
		...
		{
			$CI =& get_instance();
			$values['language'] = $CI->saisai->language->default_option();
		}
		return $values;
	}
	...
	/**
	 * Common query that joins saisai_navigation_groups table info
	 *
	 * @access	public
	...
	public function _common_query($params = NULL)
	{
		...
		$this->db->select($this->_tables['saisai_navigation'].'.*, '.$this->_tables['saisai_navigation_groups'].'.id group_id, '.$this->_tables['saisai_navigation_groups'].'.name group_name');
		$this->db->join($this->_tables['saisai_navigation_groups'], $this->_tables['saisai_navigation_groups'].'.id='.$this->_tables['saisai_navigation'].'.group_id', 'left');
...
class Saisai_navigation_item_model extends Base_module_record {
}
