//saisai = top.window.initSaisaiNamespace();
...
// date picker field
saisai.fields.datetime_field = function(context){
	var o = {
		dateFormat : 'mm/dd/yy',
		firstDay : 0,
		minDate : null,
...
saisai.fields.multi_field = function(context, inline_edit){

	var comboOptions = function(elem){
		var comboOpts = {};
		comboOpts.valuesEmptyString = saisai.lang('comboselect_values_empty');
		comboOpts.selectedEmptyString = saisai.lang('comboselect_selected_empty');
		comboOpts.defaultSearchBoxString = saisai.lang('comboselect_filter');
	...
	if (inline_edit !== false){
		saisai.fields.inline_edit_field(context);
	}
}

// markItUp! and CKeditor field
saisai.fields.wysiwyg_field = function(context){

	$editors = $ckEditor = $('textarea', context).not('.no_editor, .markItUpEditor');
	var module = saisai.getModule();
	...
	var createCKEditor = function(elem){
		...
		var sourceButton = '<a href="#" id="' + ckId + '_viewsource" class="btn_field editor_viewsource">' + saisai.lang('btn_view_source') + '</a>';
		...
		// add custom configs
		config = $.extend(config, $(elem).data());
		var hasCKEditorImagePlugin = (config.extraPlugins && config.extraPlugins.indexOf('saisaiimage') != -1);
		...
		if ($('#' + ckId).parent().find('.editor_viewsource').length == 0){
			...
			$('#' + ckId + '_viewsource').click(function(e){
				...
				if (!ckInstance.hidden){
					...
					$('#' + ckId + '_viewsource').text(saisai.lang('btn_view_editor'));
					...
					$('#' + ckId + '_viewsource').text(saisai.lang('btn_view_source'))
	...
	var createPreview = function(id){
		...
		var $textarea = $('#' + id);
		var previewButton = '<a href="#" id="' + id + '_preview" class="btn_field editor_preview">' + saisai.lang('btn_preview') + '</a>';
...
// file upload field
saisai.fields.file_upload_field = function(context){
	...
	saisai.fields.asset_field(context);
}

// asset select field
saisai.fields.asset_field = function(context, options){
	...
	var showAssetsSelect = function(){
		var winHeight = 450;
		var url = jqx_config.saisaiPath + '/assets/select/' + selectedAssetFolder + '/?selected=' + escape($('#' + activeField).val());
		...
		$modal = saisai.modalWindow(html, 'inline_edit_modal', false);
		...
		$iframe.bind('load', function(){
			...
			if (this.contentWindow.parent){
				...
				if (parentWindowHeight < winHeight){
					$iframe.height(parentWindowHeight - (parseInt($('#__SAISAI_modal__').css('top')) + 20));
				}
			}
	...
	$('.asset_select', context).each(function(i){
		if ($(this).parent().find('.asset_upload_button').length == 0){
			...
			switch(assetFolder.split('/')[0].toLowerCase()){
				case 'pdf':
					btnLabel = saisai.lang('btn_pdf');
					break;
				case 'images': case 'img': case '_img':
					btnLabel = saisai.lang('btn_image');
					break;
				case 'swf': case 'flash':
					btnLabel = saisai.lang('btn_flash');
					break;
				default :
					btnLabel = saisai.lang('btn_asset');
			}
			$(this).after('&nbsp;<a href="'+ jqx_config.saisaiPath + '/assets/select/' + assetFolder + '" class="btn_field asset_select_button ' + assetFolder + '" data-folder="' + assetFolder + '">' + saisai.lang('btn_select') + ' ' + btnLabel + '</a>');
		}
	});
	...
	var showAssetUpload = function(url){
		...
		$modal = saisai.modalWindow(html, 'inline_edit_modal', true);
		// // bind listener here because iframe gets removed on close so we can't grab the id value on close
	...
	$('.asset_upload', context).each(function(i){
		if ($(this).parent().find('.asset_upload_button').length == 0){
			...
			var btnLabel = saisai.lang('btn_upload_asset');
			$(this).after('&nbsp;<a href="'+ jqx_config.saisaiPath + '/assets/inline_create/" class="btn_field asset_upload_button ' + assetFolder + '" data-params="' + $(this).attr('data-params') + '">' + btnLabel + '</a>');
		}
	});
...
saisai.fields.inline_edit_field = function(context){

	// saisai.fields.multi_field(context, false);
	...
	var editModule = function(url, onLoadCallback, onCloseCallback){
		var html = '<iframe src="' + url +'" id="add_edit_inline_iframe" class="inline_iframe" frameborder="0" scrolling="no" style="border: none; height: 0px; width: 0px;"></iframe>';
		$modal = saisai.modalWindow(html, 'inline_edit_modal', true, onLoadCallback, onCloseCallback);
	...
	$('.add_edit', context).each(function(i){
		...
		var parentModule = saisai.getModuleURI(context);
		var url = jqx_config.saisaiPath + '/' + module + '/inline_';
		
		var btnClasses = ($field.attr('multiple')) ? 'btn_field btn_field_right ' : 'btn_field';
		if (!$field.parent().find('.edit_inline_button').length) $field.after('&nbsp;<a href="' + url + 'edit/" class="' + btnClasses+ ' edit_inline_button">' + saisai.lang('btn_edit') + '</a>');
		if (!$field.parent().find('.add_inline_button').length) $field.after('&nbsp;<a href="' + url + 'create" class="' + btnClasses+ ' add_inline_button">' + saisai.lang('btn_add') + '</a>');
		...
		var refreshField = function($field){
			...
			if (!selected) return;
			var refreshUrl = jqx_config.saisaiPath + '/' + parentModule + '/refresh_field';
			...
			$.post(refreshUrl, params, function(html){
				...
				saisai.fields.multi_field(context)
		...
		$('.edit_inline_button', context).unbind().click(function(e){
			...
			if (!val){
				alert(saisai.lang('edit_multi_select_warning'));
				return false;
			}
...
saisai.fields.linked_field = function(context){
	...
	var bindLinkedKeyup = function(slave, master, func){
		...
		if (!func){
			$('#' + masterId).blur(function(e){
				var url = __SAISAI_PATH__ + '/' + module + '/process_linked';
				var parameters = {
					master_field:master, 
...
// create number field
saisai.fields.number_field = function(context, options){
...
saisai.fields.currency_field = function(context, options){
...
saisai.fields.template_field = function(context, options){
	...
	var repeatable = function($repeatable){
		...
		$repeatable.each(function(i){
			...
			options.addButtonText = saisai.lang('btn_add_another');
			options.removeButtonText = saisai.lang('btn_remove');
			options.warnBeforeDeleteMessage = saisai.lang('warn_before_delete_msg');
	...
	$(document).off('sortStopped').on('sortStarted', function(){
		saisai.fields.sortStarted();
	})
	$(document).off('sortStopped').on('sortStopped', function(){
		saisai.fields.sortStopped();
	})

	// used event namespace http://api.jquery.com/event.namespace/
	// Remove clone event	
	$(document).off('cloned.saisai', '.repeatable_container');

	// Add another event handler	
	$(document).on('cloned.saisai', '.repeatable_container', saisai.fields.clonedFunc)

}

// hack to prevent CKEditor issues
saisai.fields.sortStarted = function(){
...
saisai.fields.sortStopped = function(){
	if (typeof CKEDITOR != 'undefined'){
		// can't pass context because we do a global destroy on CKEditor fields
		saisai.fields.wysiwyg_field();
	}
}

saisai.fields.clonedFunc = function(e){
	$('#form').formBuilder().initialize(e.clonedNode);

	// Hacktastic to remove any loader icons left on from saisai.fields.block
...
// url select field
saisai.fields.url_field = function(context, options){
	
	var activeField = null;

	var showUrlSelect = function(){
		$activeField = $('#' + activeField);
		var url = jqx_config.saisaiPath + '/pages/select/?selected=' + escape($activeField.val());
	...
	$('.url_select', context).not('.no_url').each(function(i){
		if ($(this).parent().find('.url_select_button').length == 0){
			$(this).after('&nbsp;<a href="'+ jqx_config.saisaiPath + '/pages/select" class="btn_field url_select_button">' + saisai.lang('btn_select') + '</a>');
		}
	});
...
saisai.fields.block = function(context, options){
	$(context).on('change', '.block_layout_select', function(e){
		...
		if (url.length){
			...
			if (layout && layout.length){
				...
				url = jqx_config.saisaiPath + '/blocks/layout_fields/' + layout + '/' + id+ '/english/';
			}
		}
