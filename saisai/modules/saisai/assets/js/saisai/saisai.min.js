...
jQuery.include=function(file,type){if(!type)type="script";$.ajax({async:false,url:file,dataType:type,error:function(){var msg=new jqx.lib.Message("There was an error in loading the file"+file,"error");msg.display()}})};function initSaisaiNamespace(){var f;if(window.saisai==undefined)if(top.window.saisai!=undefined)f=top.window.saisai;else f={};else f=window.saisai;return f}if(typeof window.saisai=="undefined")window.saisai={};saisai.lang=function(key){return __SAISAI_LOCALIZED__[key]};
saisai.getFieldId=function(field,context){if(window.__SAISAI_INLINE_EDITING!=undefined){var val=$(".__saisai_module__",context).attr("id");var prefix=val.split("--")[0];return prefix+"--"+field}else return field};saisai.getModule=function(context){if(window.__SAISAI_INLINE_EDITING!=undefined)return $(".__saisai_module__",context).val();else return page.module};
