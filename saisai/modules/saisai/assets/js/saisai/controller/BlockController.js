...
saisai.controller.BlockController = jqx.createController(saisai.controller.BaseSaisaiController, {
	...
	add_edit : function(){
		...
		$('#no_modal').click(function(){
			var path = jqx.config.saisaiPath + '/blocks/import_view_cancel/';
		...
		$('#yes_modal').click(function(){
			var path = jqx.config.saisaiPath + '/blocks/import_view/';
			var params = $('#form').serialize();
			$.post(path, params, function(html){
				if (html != 'error'){
					var id = '#view';
