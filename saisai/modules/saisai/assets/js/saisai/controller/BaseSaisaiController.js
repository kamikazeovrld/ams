saisai.controller.BaseSaisaiController = jqx.lib.BaseController.extend({
	...
	_init : function(initObj){
		...
		this.modulePath = jqx.config.saisaiPath + '/' + this.module;
		this.inline = parseInt($('#saisai_inline').val());
...
//		this.previewPath = myMarkItUpSettings.previewParserPath;
	...
	_initLeftMenu : function(){
		...
		var ids = [];
		$('#saisai_left_panel_inner h3').each(function(i){
		...
		var showLeftNav = function(){
			$('#saisai_body').addClass('nav_show');
			$('#saisai_body').removeClass('nav_hide');
		}
		var hideLeftNav = function(){
			$('#saisai_body').addClass('nav_hide');
			$('#saisai_body').removeClass('nav_show');
		}

		$('#nav_toggle').on('click', function(e) {
			e.preventDefault();
			if ($('#saisai_body').hasClass('nav_hide')){
				showLeftNav();
	...
	items : function(){
		...
		if ($('#tree').exists()){
			var itemViewsCookieId = 'saisai_' + _this.module + '_items';
			...
			$('#toggle_list').click(function(e){
				...
				$('#saisai_notification .rearrange').show();
		...
		$('#saisai_actions').on('click', '#rearrange', function(e){
			...
			if ($(this).parent().hasClass('active')){
				...
				$('#saisai_notification .rearrange').remove();
		...
		$('a.ico_select_all').toggle(function(e){
				...
				$('a.ico_select_all').html(saisai.lang('btn_deselect_all'));
				$('#multi_delete').parent().show();
				$('.multi_delete', '#saisai_main_content').attr('checked', true);
			},
			function(){
				if (!$('#toggle_list').parent().hasClass('active')){
					$('#toggle_list').click();
				}
				$('a.ico_select_all').html(saisai.lang('btn_select_all'));
				$('#multi_delete').parent().hide();
				$('.multi_delete', '#saisai_main_content').attr('checked', false);
			});
	...
	add_edit : function(initSpecFields){
		...
		this.notifications();
		//this._submit();
		if (initSpecFields) this.initSpecialFields($('#saisai_main_content_inner'));
		this._initViewPage();
		...
		$('.replace_action').click(function(e){
			...
			$modal = saisai.modalWindow(html, 'replace_modal', true);
			
			$modal.find('iframe#replace_iframe').bind('load', function(){
				var iframeContext = this.contentDocument;
				var replacedId = $('#new_saisai_replace_id', iframeContext).val();
				
				$('#form', iframeContext).submit(function(){
					if (confirm(saisai.lang('replace_warning'))){
						return true;
					}
					return false;
				})
		...
		$('#saisai_restore_version').change(function(e){
			$.removeChecksave();
			if ($(this).val() != ''){
		...
		$('#saisai_other_items').change(function(e){
			$.removeChecksave();
			if ($(this).val() != ''){
		...
		if (jqx.config.warnIfModified) $.checksave('#saisai_main_content');
	},
	...
	_initToolTips : function(context){
		$('.tooltip', context).tooltip({
			delay: 0,
			showURL: false,
			id: '__saisai_tooltip__'
		});
		
	},
	
	_initFormTabs : function(context){
		if (!$('#saisai_form_tabs', context).length){

			var tabId = 'tabs_' + jqx.config.uriPath.replace(/[\/|:]/g, '_').substr(5); // remove saisai_
			var tabCookieSettings = {group: this.uiCookie, name: tabId, params: {path: jqx.config.cookieDefaultPath}}

			var tabs = '<div id="saisai_form_tabs" class="form_tabs"><ul>';
			...
			tabs += '<input type="hidden" name="__saisai_selected_tab__" id="__saisai_selected_tab__" value="' + startIndex + '" />';
			$legends.filter(':first').parent().before(tabs);

			$('#form').trigger('saisai_form_tabs_loaded', [$('#saisai_form_tabs')] );

			$tabs = $('#saisai_form_tabs ul', context);
			$tabs.simpleTab({cookie: tabCookieSettings});
			
			var tabCallback = function(e, index, selected, content, settings){
				$('#__saisai_selected_tab__').val(index);
			}
	...
	_initViewPage : function(){
		...
		var resizeViewPageModal = function(){
			var half = Math.floor($('#__SAISAI_modal__').width()/2);
			$('#__SAISAI_modal__').css('marginLeft', -half +'px');
		}
		$('.view_action').click(function(e){
			...
			$modal = saisai.modalWindow(html, 'viewpage_modal', false);
			
			$('#viewpage_close').click(function(){
				$('#__SAISAI_modal__').jqmHide();
				return false;
			});
			resizeViewPageModal();
			return false;
