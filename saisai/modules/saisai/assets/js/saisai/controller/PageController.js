...
saisai.controller.PageController = jqx.createController(saisai.controller.BaseSaisaiController, {
	...
	add_edit : function(){
		...
		var retreiveLayoutVarsCallback = function(){
			var context = $('#saisai_main_content_inner');
		...
		$('#layout').change(function(e){
			...
			var path = jqx.config.saisaiPath + '/pages/layout_fields/' + $('#layout').val() + '/' + $('#id').val() + '/' + $('#language').val();
		...
		$('#language').change(function(e){
			...
			window.location = jqx.config.saisaiPath + '/pages/edit/' +  $('#id').val() + '?lang=' + $('#language').val();
		})
		
		$('#view_twin_cancel').click(function(){
			var path = jqx.config.saisaiPath + '/pages/import_view_cancel/';
		...
		$('#view_twin_import').click(function(){
			var path = jqx.config.saisaiPath + '/pages/import_view/';
			var params = $('#form').serialize();
			$.post(path, params, function(html){
				if (html != 'error'){
...
//		}
		...
		$('#related_items li a').click(function(e){
			...
			$modal = saisai.modalWindow(html, 'inline_edit_modal', true, null, onCloseCallback);
