...
{
	if (typeof(window['__SAISAI_LOCALIZED__']) != 'undefined')
	{
		return __SAISAI_LOCALIZED__['markitup_' + key];
	}
...
myMarkItUpSettings = {
    ...
    previewParserPath:  "saisai/preview", // will be __SAISAI_PATH__ + "/preview" after custom field renders
	...
	markupSet:  [ 	
		...
		{name:markitupLanguage('php'), className:'saisai_var', key:'', openWith:'{$[![' + markitupLanguage('php') + ':!:]!]', closeWith:'}', placeHolder:'' },
...
myMarkItUpSettings.displayAssetInsert = function (selected, attrs, callback){
	...
	var url = jqx.config.saisaiPath + '/assets/select/' + folder + '?nocache=' + new Date().getTime();
...
myMarkItUpSettings.displayLinkEditWindow = function(selected, attrs, callback){
	var url = jqx.config.saisaiPath + '/pages/select/?nocache=' + new Date().getTime();
	if (selected) url += '&selected=' + escape(selected);
